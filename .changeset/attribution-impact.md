...
- **The non-monetary half**: `influenced` (model-invariant coverage, multi-counted by design) beside attributed; milestones documented as a convention; event-ladder funnels (`defineFunnel` `events:` stages, no CRM required) with a first-reach `funnel_progress` projection; `GET /v1/admin/funnels/:id/progression` reports per-transition conversion + velocity with exposed-vs-unexposed splits, labeled correlational.

...
An optional base-currency FX lens (`BASE_CURRENCY` + a sovereign operator-supplied `FX_RATES` sheet by default, opt-in Frankfurter/ECB rates cached in the new `fx_rates` table, or a BYO `FxRateProvider`) adds converted `revenueBase` figures and base-converted `sort=revenue` ranking to the admin group revenue views — off by default, fail-soft throughout, with the per-currency totals staying the untouched source of truth.

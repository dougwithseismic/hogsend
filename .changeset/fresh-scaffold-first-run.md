...
- **Source-neutral event-source prompt.** "Are you using PostHog?" (default yes, followed by a phc_ key + region interview nobody needs at scaffold time — the connect flow discovers the key itself via OAuth) is replaced by "Where will events come from?" with "My app code" (pre-wired `@hogsend/client`, zero config) as the default and PostHog as one option that only gates the post-deploy `hogsend connect posthog` hint. `--posthog-key` / `--no-posthog` remain as escape hatches.

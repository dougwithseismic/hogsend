...
- [ ] **5.5 Meta one-click connect** *(operator steer 2026-07-12: minimize signup steps).* VERIFIED auth tiers (live web, 2026-07-12):
  - **Tier 0 (works today, NO App Review):** client generates the CAPI token in their own Events Manager/Business Manager — Meta explicitly exempts self-generated tokens from review. This is what `createMetaCapiDestination` takes now; fine for self-hosters.
  - **Tier 1 (the one-click, accessible):** **Facebook Login for Business** — a saved login configuration (`config_id`) requesting a **Business Integration System User access token** (programmatic, long-lived) with `ads_management`/`business_management`; popup → approve → token. Serving OTHER businesses requires Advanced Access App Review + business verification of OUR Meta app.
  - **Tier 2 (platform-grade):** **Meta Business Extension (MBE)** — the embedded setup popup (what Shopify/GHL use): auto-creates the pixel selection AND provisions the system-user CAPI token in the client's Business Manager. Beta, rep-gated.
  - BUILD: engine `meta-connect` OAuth flow (signed-state pattern already exists from the Discord connector; store tokens in the existing `provider_credentials` table; `createMetaCapiDestination` resolves stored credentials with env fallback) + Studio Integrations "Connect Meta" button. Build to the seam with a Fake exchange. SEAM ASKS: create the Hogsend Meta developer app; App Review (Advanced Access `ads_management`) + business verification; request MBE access via a Meta rep for Tier 2.

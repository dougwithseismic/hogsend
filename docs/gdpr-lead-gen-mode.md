# GDPR lead-gen mode (design)

> **Status: design doc — not built.** Phase 8.3 of `docs/revenue-attribution-plan.md`.
> An **opt-in operator mode**: nothing here changes default behavior for existing
> deployments. This is engineering design informed by GDPR/PECR as practitioners
> read them — it is not legal advice, and an agency deploying it still needs its
> own DPO/counsel sign-off on their specific processing.
...
| `user_events.properties` | `fbclid`/`gclid`/`ttclid`/… on `campaign.arrived`, form payloads on `lead.submitted`, arbitrary operator properties | **click evidence** / **contact identity** |
| `link_clicks` | `ipAddress`, `userAgent` per click | **click evidence** |
...
| `webhook_deliveries`, `dead_letter_queue`, `journey_states.context`, `import_jobs` | event payload copies in transit/replay buffers | **click evidence** TTL applies (they are buffers, not ledgers) |
...
- `link_clicks`: null `ip_address` + `user_agent` past the click-evidence
...
anonymize the contact row, strip their event properties, null their click
evidence, scrub their message bodies, and write a **tombstone** row to the
...
(you'd re-contact someone who said stop). In lead-gen mode they are stored
...
createHogsendClient({
  privacy: {
    mode: "lead-gen",               // opt-in; omit = today's behavior, no reaper
    retention: {
      clickEvidence: days(90),      // IPs, user agents, click IDs, payload buffers
...
// HOGSEND_PRIVACY_MODE=lead-gen
// HOGSEND_RETENTION_CLICK_EVIDENCE_DAYS=90
...
- `clickEvidence` shorter than any conversion definition's
...
- `retention` without `mode: "lead-gen"` → error, not silent no-op: the
...
Defaults above are deliberate: 90 days of click evidence covers the default
...
3. **Should consent also gate CAPI dispatch?** Meta receives hashed PII; in
   ...
   a disclosure. Proposal: in lead-gen mode, dispatch only consented
   conversions (they're the only ones with click evidence anyway — the
   forged-value and consent gates converge).

...
- [ ] 1.2 Vanity redirect: extract the shared click pipeline from `routes/tracking/click.ts`, add root-mounted `GET /l/:slug` running it (404→redirect-home behavior matching UUID route), tests (302 + `link_clicks`/`click_count` side effects, retarget reflected, archived slug still resolves, unknown slug)
...
- [ ] 2.1 QR scan spine: partial unique index migration (`tracked_links(link_id) WHERE source='qr'`), `ensureQrTrackedLink()`, `aggregateFor` FILTER split (`scanCount`, canonical `trackedLinkId`, `qrTrackedLinkId`), `scanCount` in admin link responses — with tests (lazy mint idempotent under race, scan increments scanCount not conflated, retarget covers QR row)

...
| **convert** | any `conversions` row for a referee with a bound touch (hooked next to `recordAttributionCredits`) | none (a fact) | `attribution_credits.referral_touch_id` | `referral.converted` → direct referrer; `referral.tree_converted` → each ancestor to depth 5 with `{ level, value, currency, refereeContactId }` |

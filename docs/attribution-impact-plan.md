...
- [x] **1.5 Admin + Studio: revenue by journey/campaign/template per model.** Extend `GET /v1/admin/attribution` with `groupBy=journey|campaign|channel|template` (+ existing model × currency); Studio Impact surface shows a journey list with attributed value per selected model, drill-down to per-template. Journey detail page gets a revenue panel.
...
- [x] **5.1 Attribution backfill command.** DECIDED (open Q5): admin endpoint (`POST /v1/admin/attribution/backfill`, batch-shaped with a two-stage cursor) + `hogsend attribution backfill` CLI looping it — the code-first registry only exists in the API process, so the CLI drives it remotely. Recompute requires a definitionId, deletes credits only (never conversion rows), logged loudly. Backfill never re-fires ad-platform dispatches and never mints contacts. `hogsend attribution backfill [--definition X] [--since date]`: replays historical `user_events` through conversion evaluation + ledger writing (both already idempotent: unique `(definitionId, eventId)` and `(conversion, model, touchpoint)`). An EXISTING deploy that upgrades gets its history credited in minutes; a definition/window change gets a clean recompute path (delete-then-refill scoped to a definition, guarded + logged).
- [x] **5.2 Zero-config revenue conversion.** `trigger.event: "*"` wildcard on ConversionRegistry + built-in `revenue` definition (value > 0, quote events excluded via the new `event`-name injection into `where` eval, browser events excluded by the default sources gate). Seeded unless the consumer authors `id: "revenue"` or sets `HOGSEND_DEFAULT_REVENUE_CONVERSION=false`. Ship a default built-in conversion definition (`revenue`, enabled unless disabled): any server-source event with `value > 0` fires it. A fresh adopter who ingests one Stripe/order webhook sees the Impact tab populate with NO configuration. The seeded PostHog destination pattern is the precedent for opt-out defaults.
- [x] **5.3 First-week checklist + docs.** `apps/docs/content/docs/conversions/impact.mdx` — day 0/1/3/7 checklist, the three-number reading guide (claim-strength table), backfill recipe, windows-forward-only note. `/docs/impact` guide: instrument arrival capture (`@hogsend/js` — 2 min), point one revenue webhook at a source (10 min), optionally define one funnel + one milestone; what you see on day 1 (touches, influenced), day 3 (first credits), day 7 (per-journey attributed + first lift read if holdout on). The adoption promise, written as a checklist we can defend.
- [x] **5.4 Readiness surface.** Four optional-tier rows on `GET /v1/admin/readiness` (arrival capture / valued events / conversions / credits), each naming the exact missing wire; never blocks setup-`ready`. Extend the existing readiness/admin checks: "attribution readiness" = arrival capture seen? valued events flowing? conversions firing? credits accruing? — so "how's it doing" has an answer even when the answer is "not wired yet, here's the missing wire."
...
3. ~~Does `sms_sends` need `campaign_id` now or at seam?~~ ANSWERED at 1.1: campaign steps are `send | wait` only (`packages/core/src/types/campaign.ts`) — no SMS steps exist today, so `sms_sends.campaign_id` waits for campaigns phase 2 (channel steps).

...
- **`verify-scaffold.sh`** stays the **manual** pre-release harness (`pnpm --filter create-hogsend verify`) — it builds + packs the real tarballs and scaffolds a fresh app. It is NOT a per-PR CI gate: in CI its internal per-package builds intermittently leave no `dist/` ("missing package/dist/**") in a way that doesn't reproduce locally. Run it by hand before a release; the doctor + the post-publish registry check cover the version-line + publish-existence invariants in CI. (Open task: make verify-scaffold CI-safe — likely needs the workspace pre-built and its own `pnpm --filter … build` calls made robust on a cold runner.)

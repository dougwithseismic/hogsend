...
**The discipline now:** each release, write a changeset that EXPLICITLY bumps all ELEVEN engine-line packages (`engine, db, core, cli, client, email, plugin-posthog, plugin-resend, plugin-postmark, studio` + the bare `hogsend` CLI alias) to the same bump type. They all start uniform, so they land uniform. `verify-scaffold.sh` catches drift in the nine *scaffold-pinned* packages, and `release-doctor`'s "all engine-line packages share one version" check — which derives its set from disk (every publishable `@hogsend/*` package PLUS the bare `hogsend`) — catches drift across all eleven, including opt-in `plugin-postmark`.

🚨 **The bare `hogsend` package (packages/hogsend) is the un-scoped CLI alias** — bin forwards to `@hogsend/cli/bin` so `npx hogsend`/`pnpm dlx hogsend` work. It rides the engine line, is NOT scaffold-pinned (leave it out of the three scaffold lists), and being un-scoped it needed its own manual first publish + token/OIDC coverage like any new package.

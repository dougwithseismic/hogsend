...
| --- | --- | --- | --- | --- |
| 01 | [Provider contract + presets](prds/01-provider-contract.md) | `[ ]` | — | `@hogsend/core`: `defineAccountLink`, `LinkedIdentity`, `AccountLinkHooks`, `oauth2Link()`, `steamOpenIdLink()`. Types + pure functions, zero DB |
...
| 04 | [Merge + adopt repointing](prds/04-merge-repoint.md) | `[ ]` | 02 | Add `linked_accounts` to the merge repoint list and `adoptOrphanHistory`; handle the singleton-collision case explicitly |
...
| 11 | [Player manage + revoke page](prds/11-manage-page.md) | `[ ]` | 09, 10 | `GET /v1/accounts/manage?token=`, unsubscribe-token idiom, per-row revoke |
...
**The known seam (PRD 07, 16).** Real OAuth credentials for Steam, Discord and Twitch. Every provider
gets a deterministic Fake so the whole flow is testable end to end without them; the human ask is a
Steam Web API key and Discord/Twitch app client id + secret. Build to the seam, mark `[~]`, keep
going.

...
| 23 | [Fresh scaffold does not type-check](prds/23-scaffold-typecheck-consumer.md) | `[x]` | 12 | **SOLVED 2026-08-12 — the bug was NOT ours.** `@hono/zod-openapi@1.5.2` ships broken declarations (imports only `{ ZodError, ZodType }` from zod, then `import z = zodModule.z` with `zodModule` never declared), so `z` is `any` and all 33 errors cascade from it. Eight hypotheses died first because `skipLibCheck` ACTIVELY HIDES the `TS2503` that names it. We were protected by our lockfile alone — the engine declares a caret and a scaffold ships no lockfile, so a fresh install floated to latest; `pnpm update` would have broken us identically. Pinned to 1.4.0 (1.5.0 dropped the `types` export condition and never restored it, so 1.5.1 is clean for this bug and worse for another). The template also ships an `overrides` block, which is the only part that unbreaks a scaffold against an engine ALREADY on npm. Verified end-to-end against published `@hogsend/engine@0.63.0`: `check-types` exit 0. Remove the pins when upstream fixes it. |

...
**Resuming after a break? Read [HANDOVER.md](HANDOVER.md)** — state, runbooks, and the lessons that cost real time.
...
| 16 | [Inbound replies](prds/16-inbound-replies.md) | `[~]` | 06, 15 | **ALL 7 TASKS SHIPPED 2026-08-12, and then made REACHABLE.** Replies land as `email.replied` on the outbound spine; forwarding to a human address is mandatory (inbound cannot be enabled without one) and deliberately not a gate — a failed forward records `forward_error` and still stores and emits, so `forwarded_at IS NULL AND forward_error IS NOT NULL` is the operator's re-drive list. Auto-responders are stored and forwarded but never emitted, so an out-of-office cannot advance a journey or start a mail loop. The service had NO production caller until `POST /api/email/domains/inbound` landed — every piece existed and none could be reached. **Seam:** a live inbound test needs the MX published on a domain we control (`hogsend.com` is on Cloudflare, so self-serviceable). The relay IAM seam is CLOSED — `aws-bootstrap-events.sh` grants `SesInboundReceiptRules`. |
...
| 21 | [Fake association gaps](prds/21-fake-association-gaps.md) | `[x]` | 11, 14 | The SECOND live walkthrough (2026-08-11) found 20/22 verbs clean — PRD 14's corrections HOLD. The three that disagreed share ONE root: the Fake's `associateResource` accepted a resource that does not exist, so a walkthrough bug (deriving an email-address sender's ARN from its parent domain) produced an association AWS 404s and a send it 403s. **Correction:** an earlier version of this row claimed the Fake sends without association — it does not, PRD 14 added that rule in `7b01b124`; it was satisfied by a seeded fiction. |
| 22 | [Half-provisioned domain never sends](prds/22-unassociated-domain-self-heal.md) | `[x]` | 07, 21 | **PRODUCTION BUG.** `ses-domains.ts create()` returns early when the identity exists, 40 lines above the `associateResource` call. A provision that dies in between leaves the domain permanently unable to send (AWS 403, observed live), reporting itself healthy, and a retry short-circuits so it never heals. Found while fixing PRD 21 — the failure PRD 21 wrongly attributed to the Fake turned out to be real, in production code. |
| 23 | [Fresh scaffold does not type-check](prds/23-scaffold-typecheck-consumer.md) | `[x]` | 12 | **SOLVED 2026-08-12 — the bug was NOT ours.** `@hono/zod-openapi@1.5.2` ships broken declarations (imports only `{ ZodError, ZodType }` from zod, then `import z = zodModule.z` with `zodModule` never declared), so `z` is `any` and all 33 errors cascade from it. Eight hypotheses died first because `skipLibCheck` ACTIVELY HIDES the `TS2503` that names it. We were protected by our lockfile alone — the engine declares a caret and a scaffold ships no lockfile, so a fresh install floated to latest; `pnpm update` would have broken us identically. Pinned to 1.4.0 because it is the only version this suite has ever run against, on a launch day, when the whole benefit of the newer 1.5.1 was "closest to latest". (1.5.0 also dropped the `types` export condition and never restored it — but that is a robustness regression, NOT a live break: TypeScript still resolves `import` → `dist/index.mjs` and picks up the sibling `index.d.mts` under both `Bundler` and `node16`. A margin argument, not evidence 1.5.1 would fail.) The template also ships an `overrides` block, which is the only part that unbreaks a scaffold against an engine ALREADY on npm. Verified end-to-end against published `@hogsend/engine@0.63.0`: `check-types` exit 0. Remove the pins when upstream fixes it. |
...
| 14 | ~~A real SNS topic + `sns:` actions on the relay policy, so `putEventDestination` can be proven.~~ **DONE 2026-08-11.** `putEventDestination` ran against real AWS in the PRD 19 proof; delivery, bounce and complaint all travelled the full pipeline. | done |
...
| 12 | ~~INVESTIGATE the scaffold-smoke divergence~~ **DIAGNOSED 2026-08-11, and the recorded description was wrong.** Reproduced locally: 33 errors, all implicit-any inside the engine's shipped source, no zod anywhere. The smoke is not flaky — it is correctly failing on a real consumer-facing defect. Queued as PRD 23. Original text: `pnpm --filter create-hogsend verify` fails locally at step 5 with 33 zod `.refine` inference errors, at the branch base as well as on the branch, with and without a prior `pnpm build` — while CI ran the same step on the same commit and passed. Until this is understood, a green CI is not evidence that a published scaffold type-checks. PRD 12's own assertion has never executed. | open |
| 11 | ~~RUN the walkthrough once the AWS account exists.~~ **DONE — run twice on 2026-08-11.** The first found 3 divergences (one root: the probe's sender-ARN derivation); the second, after PRD 21, compared **22 verbs with 0 divergences and 0 resources left behind**. | done |
...
| — | FLAKY TEST #2, with a DIAGNOSED mechanism (unlike the one below): `apps/cloud/src/__tests__/ops-stats.test.ts > readOpsStats` — both `zero-fills every enum key on an empty fleet` and `counts stacks, orgs, errors, alerts and builds from seeded state` failed once on 2026-08-11, then passed alone (7/7) and on a forced full re-run (1579/1579). **`readOpsStats` reads GLOBAL fleet counts while every other file seeds rows into the same shared Postgres**, so 'an empty fleet' is only ever true when nothing else is running. Any test that adds a seed makes it likelier; PRD 22 added one. The fix is to scope the assertion to rows the test owns, not to retry it. Recorded because a global-count assertion under concurrency fails at random and gets blamed on whatever change is in flight. | open |
...
| 15 | **NARROWED 2026-08-11.** The API-contract half is CONFIRMED: the walkthrough's own identity is a three-level subdomain (`<runid>.ses-walkthrough.hogsend.com`) and `createIdentity` accepted it and returned BYODKIM tokens on both live runs, so a subdomain verifies exactly like a root domain. What remains unproven is the DNS half — publishing the TXT record and watching a subdomain reach `verified`, then sending from it. That needs a **Cloudflare API token scoped to the hogsend.com zone (Zone → DNS → Edit) plus the zone id**, which is the only outstanding ask for it. | open |

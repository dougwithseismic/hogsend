...
| 15 | Decide whether the branded return path should be ON by default. Doug asked for MX+SPF by default and the stated reason was REPLIES, which the return path does not deliver (that is PRD 16). One-line change either way; deliberately not made silently. | open |
</content>

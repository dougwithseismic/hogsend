...
| 23 | [Fresh scaffold does not type-check](prds/23-scaffold-typecheck-consumer.md) | `[ ]` | 12 | **CONSUMER-FACING.** `pnpm dlx create-hogsend@latest` produces an app that fails `pnpm check-types` with 33 errors, ALL of them inside `@hogsend/engine`. PRD 12's seam called them "zod `.refine`" errors; they are implicit-any (TS7006/TS7053). The engine ships raw `.ts`, so consumers type-check our IMPLEMENTATION and `skipLibCheck` cannot help. Strictness and moduleResolution both ruled out by experiment. |

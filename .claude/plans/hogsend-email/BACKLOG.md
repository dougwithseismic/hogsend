...
| 21 | [Fake association gaps](prds/21-fake-association-gaps.md) | `[ ]` | 11, 14 | The SECOND live walkthrough (2026-08-11) found 20/22 verbs clean — PRD 14's corrections HOLD — but the Fake **sends happily from an identity the tenant does not own**, which real SES refuses with a 403. Every send test in the repo passes without the association production requires. Also: the Fake associates a NON-EXISTENT identity, and the walkthrough derives an email-address sender's ARN from its parent domain. |

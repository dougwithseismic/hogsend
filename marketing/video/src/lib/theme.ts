/**
 * Hogsend brand tokens — EXACT values from the apps/docs design system
 * (app/global.css + components/ds). Do not invent new colours; every
 * scene draws from this palette.
 */
export const theme = {
...
/**
 * Type metrics from the docs site, scaled for video. Letter-spacing is
 * the signature: hero −0.06em, everything else −0.02em.
 */
export const typo = {
...
/** Border radii from the docs DS (multiply by fontScale in components). */
export const radius = {
  /** Cards, chips — `rounded-md` */
  card: 6,
  /** Code mocks, buttons — `rounded-[10px]` */
  mock: 10,
  /** Glass panels — `rounded-xl` */
  panel: 12,
} as const;

/**
 * Syntax palette for CodeScene — the EXACT github-dark Shiki colours the
 * site's code blocks render with (sampled from hogsend.com), plus the

...
<Callout type="info">
  These are the **admin/ops** contact endpoints (list, search, inspect — admin-scoped). To **write** contacts from your application code, use the public data plane: [`PUT /v1/contacts`](/docs/data-api/contacts) (upsert), `GET /v1/contacts/find`, and `DELETE /v1/contacts`, authed with an `ingest`-scoped key.
</Callout>

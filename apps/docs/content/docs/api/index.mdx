...
Studio and user-facing endpoints use [Better Auth](https://www.better-auth.com/) with email/password authentication. Auth routes are mounted at `/api/auth/*`:
...
| `STUDIO_ADMIN_EMAIL` | No | -- | Set it to env-bootstrap the first Studio admin on a zero-user DB (see [Authentication](#authentication)) |
...
| Page | Description |
|---|---|
| [Ingestion & Webhooks](/docs/api/ingestion) | Event ingestion, provider email webhooks, generic webhook sources, health endpoint |
...
| [Buckets API](/docs/api/buckets) | Bucket listing with counts, detail with criteria and fed journeys, member listing, enable/disable |

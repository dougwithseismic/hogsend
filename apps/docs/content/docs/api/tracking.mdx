...
Reserved namespaces (`email.`, `journey.`, `bucket.`, `contact.`) are rejected for semantic event names at send time, properties are scalars-only and size-capped, and a semantic link must have an absolute http(s) `href`.

...
2. **Env bootstrap:** set `STUDIO_ADMIN_EMAIL` (+ optional `STUDIO_ADMIN_PASSWORD`, min 8) and the **API process** mints it on boot — but **only** into a zero-user table (idempotent on a fresh DB). When `STUDIO_ADMIN_PASSWORD` is unset, a strong password is auto-generated and printed once to the server log; rotate it via the Studio reset flow. See [Operating → Studio](/docs/operating/studio#create-the-first-admin-two-ways).

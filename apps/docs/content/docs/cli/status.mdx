...
<Callout type="info">
`hogsend status` reads only the `status`, `version`, and `uptime` fields from `/v1/health`. It does **not** yet surface the two-track migration schema block (`schema.engine` / `schema.client`) or the `migration_pending` status that the health endpoint now returns. To inspect migration state, call the [health endpoint](/docs/api/ingestion#health) directly.
</Callout>

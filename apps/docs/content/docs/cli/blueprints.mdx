...
| `--allow-reenrollment` | Required alongside `--journey-id` when the new id differs from the blueprint id. The blueprint id **is** the journey id, so renaming it discards the `entryLimit: "once"` history and re-enrolls (re-emails) everyone who already completed the blueprint — pass this to do it intentionally. |
| `-h, --help` | Show this help. |

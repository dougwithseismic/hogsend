...
`pnpm bootstrap` is idempotent and auto-remaps any busy host ports — set `RESEND_API_KEY` in `.env` before sending real email. That's it: your app pins the engine, owns its journeys, and upgrades with `pnpm up`.

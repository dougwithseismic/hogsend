---
title: Introduction
description: A TypeScript framework for growth teams that code. Identify and enrich the people on your product, react to their events, message them where they are, and measure what moved.
---

## What is Hogsend?

**Superpowers for growth teams that code.**

<Callout title="TL;DR">
One command scaffolds the whole stack, one command runs it: `pnpm dlx create-hogsend@latest my-app`, then `pnpm hogsend dev` — API, worker, Studio, and database in a single terminal. Already on PostHog? Point a webhook at your app and the events you capture today start triggering journeys.
</Callout>

Hogsend is a TypeScript framework for growth engineering. Identify and enrich the people on your product, react to the events they fire, message them where they are — email, SMS, Discord, Telegram — and measure what each journey moved. Buckets, tracked links, feature flags, funnels, groups, account linking, and a Studio to watch it all come built in.

Under it all is **one loop**: an inbound event comes in, your code reacts, engagement flows back out. A call from your own app, a Stripe event, a PostHog webhook — they all normalize to an event for a user. Your TypeScript reacts (journeys send/wait/branch, buckets segment in real time), and everything that happens — opens, clicks, sends, journey completions — fans back out to your analytics, CRM, or warehouse.
...
3. **Identity gets joined up** — One person arrives under many keys: an anonymous id, an email, a `userId`, and on a game a platform account. [`defineAccountLink()`](/docs/guides/account-links) binds their Steam or Twitch account to the contact, so a SteamID and an email are the same person to every journey, bucket and email after it. Steam needs no credentials at all.
4. **Your code reacts** — TypeScript functions that trigger on events, send emails, wait, branch, and adapt based on what the user does.
5. **Email sends through a swappable provider** — [Resend](https://resend.com) by default, [Postmark](/docs/guides/email) opt-in, or any other behind the `EmailProvider` contract. React Email templates with first-party open/click tracking, unsubscribe management, and deliverability monitoring — all engine-owned, so they come along with any provider.
6. **Engagement flows back out** — Opens, clicks, sends, journey completions, and bucket transitions fan out to PostHog, Segment, Slack, a CRM, or a warehouse via [outbound destinations](/docs/data-api/destinations).
...
- **Mints two API keys** — an ingest-scoped `HOGSEND_API_KEY` (`hsk_…`) so the [data API](/docs/data-api) and [`@hogsend/client`](/docs/data-api/client-sdk) work out of the box, and a full-admin `HOGSEND_ADMIN_KEY` that powers the [`hogsend` CLI](/docs/cli).
...
- **Account linking** — Bind a player's Steam or Twitch account to a contact with [`defineAccountLink()`](/docs/guides/account-links), so the platform id and the email are the same person. Steam needs no credentials at all. Every link carries a monotonic version, so your own database can mirror it exactly.

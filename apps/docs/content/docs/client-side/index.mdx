...
- **[Script tag](./script-tag)** — no bundler, no React: one `<script src="…/hogsend.js">` served by your engine.
...
- **[Personalization](./personalization)** — `useContact` / `useTrait` over the operator-allowlisted contact traits.
...
`GET /v1/contacts/me` reads the same way: the recipient is resolved server-side from the `userToken` (or the caller's own anonymous id), and it returns only the properties the operator put on the engine's `contacts.publicProperties` allowlist, which is empty by default. See [Personalization](./personalization).

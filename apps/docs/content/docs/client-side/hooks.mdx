...
The operator-allowlisted projection of the resolved contact's traits.

```ts
interface UseContactResult {
  traits: Record<string, unknown>;   // only keys on contacts.publicProperties
  identified: boolean;               // contact carries an externalId or email
  email: string | null | undefined;  // undefined unless contacts.exposeEmail
  loading: boolean;                  // true until the first fetch resolves
}
```
...
const { traits, identified, loading } = useContact();
const plan = useTrait("plan"); // re-renders only when this trait changes
```

Neither hook fetches. The SDK loads `GET /v1/contacts/me` on init, on identity change, and after `identify()` resolves, and these hooks select against that slice. The allowlist is empty by default, so both read empty until an operator configures `contacts.publicProperties`. See [Personalization](./personalization).
...
interface UsePreferences {
  ...
  lists: ListSummary[];                   // the GET /v1/lists catalog, each list's `subscribed` overlaid live; ListSummary.kind is "channel" | "topic" | undefined
  loading: boolean;
  ...
  setUnsubscribedAll: (unsubscribed: boolean) => Promise<void>;  // global email opt-out → POST /v1/lists/preferences
...
`get()` reads `GET /v1/lists/preferences`; `subscribe`/`unsubscribe` hit `POST /v1/lists/{id}/subscribe` / `unsubscribe`. Every `setPreference`/`subscribe`/`unsubscribe` emits `inapp.preference_changed` with `{ categoryId, subscribed }`. `setUnsubscribedAll(unsubscribed)` writes the account-wide email master toggle via `POST /v1/lists/preferences` and emits `inapp.preference_changed` with `categoryId: "$all"` (the exported `ALL_EMAILS_CATEGORY` sentinel) plus `scope: "all"`. Writes require an identified user; an anonymous caller falls to `anonymousId`, which the engine rejects for preferences.

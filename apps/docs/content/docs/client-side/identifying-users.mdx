...
Reading back works the same way: `GET /v1/contacts/me` resolves the recipient from the `userToken` (or the caller's own anonymous id) and returns the operator-allowlisted traits, never a contact named in the request. See [Personalization](./personalization).

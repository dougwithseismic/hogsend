...
- **`createHogsendClient({ journeys, email?: { provider?, templates? }, analytics?, enabledJourneys?, overrides? })`** — build the client (the shared services bag) from your journeys and content. `email` ({ `provider`, your `src/emails` `templates` registry }) and `analytics` (PostHog by default) are first-class; `overrides` is a small advanced/test-only escape hatch (replace the whole mailer, swap auth/Hatchet/db).

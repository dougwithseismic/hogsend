...
The only thing you bring yourself is a `RESEND_API_KEY` when you're ready to send real email. Everything else is set up for you.
...
- **`createHogsendClient({ journeys, buckets?, lists?, destinations?, email?: { provider?, templates? }, analytics? })`** — build the client (the shared services bag) from your content. `email` carries your `src/emails` `templates` registry plus a swappable `provider`; `analytics` is PostHog by default; `buckets`, `lists`, and `destinations` are first-class arrays. An `overrides` field is a small advanced/test-only escape hatch (replace the whole mailer, swap auth/Hatchet/db).

...
Because you register journeys and sources in **your own** files — not by editing a shared engine index — there is no merge surface. Upgrading the framework is `pnpm up "@hogsend/*"` plus a migration run, **never a git merge of a fork**.

## You scaffold; you don't fork
...
<Callout type="info">
  The `@hogsend/*` packages aren't published to npm yet (everything is at `0.0.1`; the publish pipeline is staged but hasn't run). `pnpm dlx create-hogsend@latest` is the intended install path — available imminently. Today the packages are consumed from this monorepo.
</Callout>
...
- **Eject** — `hogsend eject <package>` (the Node `@hogsend/cli`, distinct from the Go `hogsend` deployment CLI) copies one package into `vendor/` and rewrites just that dependency to a local file link. You maintain a fork of **that one package**; everything else still `pnpm up`s.

...
trigger: { bucket: powerUsers },        // desugars to the line below
exitOn:  [{ event: powerUsers.left }],  // "bucket:left:power-users"

// …or name the transition yourself. Equally canonical, equally checkable:
trigger: { event: powerUsers.entered }, // "bucket:entered:power-users"
```

`trigger: { bucket }` hands over the bucket OBJECT and lets the engine derive the
event. It resolves to `bucket.entered` at definition time, so what gets stored,
registered, routed by Hatchet and drawn in Studio is the same plain event string
either way — nothing downstream learns a second concept. Declare `event` **or**
`bucket`, never both and never neither; both mistakes throw at `defineJourney`
rather than quietly picking one for you.

There is no `{ bucket }` form for `exitOn`, which takes a list of transitions
rather than a single trigger — spell those with `bucket.left` as above.

`bucket.entered` is the literal type `` `bucket:entered:${Id}` `` and `bucket.left` is `` `bucket:left:${Id}` ``, both derived synchronously from the bucket's own id when `defineBucket()` runs. They're plain strings at runtime — byte-identical to the events the engine emits — but TypeScript ties them to the bucket, so you can't reference a bucket that doesn't exist or a transition you spelled wrong. (`trigger.event` is typed `string`, so a bare string literal would compile and silently route nothing; the ref makes the wire checkable. `trigger: { bucket }` gets there a second way — the `bucket` key demands the `bucket:entered:` shape, so a hand-rolled `{ entered: someConfigValue }` is a compile error too.)
...
A **dynamic** bucket (the default) recomputes membership from `criteria` automatically. You write the predicate; the engine maintains the set — joins and most leaves inline as events arrive, time-based leaves via the reconcile cron.

A bucket declared `kind: "manual"` registers like any other, and carries the same membership-age knobs — `entryLimit`, `entryPeriod`, `minDwell`, `maxDwell`. What it does not have is a predicate: it is never recomputed, never backfilled, and never re-evaluated when an event arrives. Its membership changes only when something explicitly adds or removes a member — `POST /v1/buckets/:id/members` and `DELETE /v1/buckets/:id/members/:userId`, the secret-key data plane. That covers a hand-curated beta cohort, a CSV import, or an audience an external system owns. Everything downstream is unchanged: joins emit `bucket:entered:<id>` subject to `entryLimit`, leaves emit `bucket:left:<id>` subject to `minDwell`, `maxDwell` still force-leaves on the reconcile cron, and journeys bound to those events don't know or care which kind produced them.

<Callout type="warn">
  A manual bucket must not declare `criteria` — that combination is rejected at registration. Nothing would ever evaluate the predicate, so the bucket would silently behave as if it were absent. Pick one: `kind: "dynamic"` with `criteria`, or `kind: "manual"` with the membership write path.
</Callout>
...
For authoring detail — re-entry policy, anti-flap (`minDwell` / `maxDwell`), the full `defineBucket()` reference, and the registration steps — see the [Buckets guide](/docs/guides/buckets).

## Next steps

<Cards>
  <Card title="Buckets guide" href="/docs/guides/buckets" />
  <Card title="Conditions" href="/docs/guides/conditions" />

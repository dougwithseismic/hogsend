...
> Want to see it instead of read about it? `pnpm dlx create-hogsend@latest my-app` scaffolds an app and brings up the full local stack for you in one run. See [Installation](/docs/getting-started/installation).
...
- **A clean migration path** — if you outgrow Hogsend before PostHog ships their solution, you can migrate to Customer.io, Brevo, or any other platform. Your event model and journey logic translate directly.
...
PostHog forwards each event to `POST /v1/events`, the same [data-plane endpoint](/docs/data-api/events) your own app code can call. Hogsend stores the event, routes it to any journey whose `trigger.event` matches, and upserts the contact. See [Connect PostHog](/docs/integrations/posthog/connect) for the wiring — `hogsend connect posthog` provisions it in one command.

### Person properties enrich journeys

Hogsend pulls PostHog person properties (with Redis caching) so your journeys and timezone logic can use the full user profile. The active provider is exposed as `getAnalytics()` — vendor-neutral, and `undefined` when no provider is configured (for PostHog, when `POSTHOG_API_KEY` isn't set) — so guard it:

```ts
import { getAnalytics } from "@hogsend/engine";

// getAnalytics() returns undefined when no provider is configured, so guard it.
const properties = await getAnalytics()?.getPersonProperties(user.id);
...
Getting there is one command. `pnpm dlx create-hogsend@latest my-app` scaffolds a thin app that pins `@hogsend/engine` and holds your content, then runs local setup itself: checks Docker, writes `.env` with a fresh `BETTER_AUTH_SECRET`, brings up Postgres + Redis + hatchet-lite, auto-mints your Hatchet token, runs the migrations, and mints your API keys — all into `.env`. Set `RESEND_API_KEY` before sending real email, point your PostHog webhook at the app, and your event stream starts driving journeys.

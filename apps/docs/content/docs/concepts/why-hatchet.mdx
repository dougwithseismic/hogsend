...
Scaffold a fresh app — local setup runs as part of it — then start the dev stack. You do not bring your own Hatchet token for local development — setup mints it.
...
The scaffolder's setup step is the same script as `pnpm bootstrap` (run that yourself if you skipped it during create), and it is idempotent — safe to re-run. It runs eight steps for you:
...
3. **Resolves ports** — if `5434`/`6380`/`7077`/`8888`/`3002` are taken it remaps to the next free port and syncs them back into `.env`.
...
6. **Runs migrations** — `db:migrate`, engine track then client track — and verifies the schema reached HEAD.
7. **Mints two API keys** — an ingest-scoped `hsk_…` written to `HOGSEND_API_KEY`, so the [data API](/docs/data-api) and [client SDK](/docs/data-api/client-sdk) work the moment you boot, plus a full-admin `HOGSEND_ADMIN_KEY` for the [`hogsend` CLI](/docs/cli).
8. **Offers to create your first Studio admin** (interactive terminals only; sign-up is closed by design).

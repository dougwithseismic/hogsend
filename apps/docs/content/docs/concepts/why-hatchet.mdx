...
Email sends and other side effects run as Hatchet tasks with retries and exponential backoff. If Resend returns a 500 or times out, Hatchet retries automatically. Non-retryable errors (invalid API key, malformed email) bail out immediately.
...
|---|---|---|
| Event ingestion | You build it | `POST /v1/events` + `defineWebhookSource()`; PostHog connects via the [Destinations pipeline](/docs/getting-started/posthog-setup) |

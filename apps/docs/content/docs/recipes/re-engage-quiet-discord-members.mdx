...
<Callout type="warn">
  This journey triggers on `discord.presence_active`, which requires the `GUILD_PRESENCES` intent. The gateway worker does **not** request it by default, so the journey never fires until you [opt in](/docs/integrations/discord#presence-is-opt-in). Trigger on `discord.message_sent` instead if you would rather not subscribe to the presence feed: the `last_seen` math below is identical either way.
</Callout>

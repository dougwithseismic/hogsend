...
description: Loops' POST /transactional → Hogsend's hs.emails.send({ to, template, props }). Verify-email, password-reset, magic-link, and receipt — with typed props, automatic tracking, and unsubscribe handling.
---

A transactional email is a one-off, system-triggered message to a single person: verify your address, reset your password, here's your magic link, here's your receipt. In Loops you `POST /transactional` with a `transactionalId` and `dataVariables`. In Hogsend you call:

```ts
await hs.emails.send({ to, template, props });
```

`template` is a key in your own React Email registry (the equivalent of Loops' `transactionalId`), and `props` are the template's variables (Loops' `dataVariables`) — except they're **type-checked**.
...
await loops.sendTransactionalEmail({
  transactionalId: "clfq...",
  email: "ada@example.com",
  dataVariables: { firstName: "Ada", url: "https://app.example.com/verify?t=…" },
});
...
The Loops opaque `transactionalId` becomes a readable, typed registry key. `props` is autocompleted and prop-checked against the template you reference — pass an unknown key and `tsc` fails before you ship.
...
This is the concrete win over Loops' untyped `dataVariables`: a renamed or missing prop is a build failure, not a broken email in production. (Without `@hogsend/email` the shape degrades to `{ template: string; props? }` — see the [client SDK reference](/docs/data-api/client-sdk).)
...
This means a transactional open or click is a first-class event in the engine — it can trigger a journey or move bucket membership (a "clicked the receipt but never returned" segment, say). Loops does not expose tracking on transactional mail at all.

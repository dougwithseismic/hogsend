...
The `retention-weekly-digest` template ships in the example app ([see it rendered](/emails)) — copy it into your `src/emails/` with a `registry.ts` entry and a `templates.d.ts` augmentation ([Email guide](/docs/guides/email)); after that, the `props` bag above is type-checked. Register the journey by adding `weeklyDigest` to your `journeys` array, exactly as in [Lifecycle journeys](/docs/recipes/lifecycle-journeys#register-the-journeys).

...
description: Loops' Campaigns → Hogsend's hs.campaigns.send({ list, template, props }). Define a list, subscribe contacts, and broadcast marketing/product-update — list vs bucket audiences, polarity, and the durable/idempotent/preference-checked send.
---

A marketing campaign is a one-time broadcast of a single template to a whole audience. In Loops you create a Campaign in the UI, pick an audience, and hit send. In Hogsend you call:

```ts
await hs.campaigns.send({ list: "product-updates", template: "marketing/product-update", props });
```

It queues a durable broadcast that sends the template to every subscribed member of the list, asynchronously, in the worker. Exactly one of `list` or `bucket` is the audience.

## Loops → Hogsend

```ts
// Loops: create a Campaign in the UI → pick the "Product updates" audience → send

// Hogsend: one call, audience is a code-defined list
...
Flip a contact's membership on. In Loops you'd toggle a mailing list on the contact; here it's a one-liner (or part of an `upsert`/`events.send` call via the `lists` bag).
...
| Loops analogue | A mailing list | A filtered audience |

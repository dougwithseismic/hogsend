...
- **It resumes on the link.** Fire `contact.linked` once the member is emailable, and the wait resolves the instant it lands — the welcome arrives in minutes, not at the next timer tick. Emit it with `ctx.trigger` from inside a journey: `ctx.trigger` injects the registry/Hatchet/logger the ingest pipeline needs, which a bare connector callback in `src/discord.ts` cannot reach (a raw `ingestEvent()` call requires all four — `{ db, registry, hatchet, logger, event }`).
...
export const emitContactLinked = defineJourney({
  meta: {
    ...
    // (the /link email-confirm flow folded the address onto the discord_id).

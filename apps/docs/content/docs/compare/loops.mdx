...
**From $49/mo** | contact-based pricing -- free up to 1,000 contacts ($49 at 5k, $99 at 10k, $149 at 15k)

*Pricing last verified 5 June 2026 -- vendors change plans often, so check [Loops' pricing page](https://loops.so/pricing) for the current numbers.*
...
Automation in Loops is deliberately simpler than Customer.io or ActiveCampaign. You get welcome sequences, time delays, and basic branching, but you will hit the ceiling on heavy conditional logic, fine-grained event-property filtering, and cross-journey orchestration. For "welcome sequence plus trial nudge" that ceiling is fine. For "branch on three runtime conditions, call an internal API mid-flow, then route into a second journey," it starts to feel constraining.

There is no self-hosting. Your contact data lives on Loops' infrastructure, and pricing is contact-based -- you pay as your list grows, whether or not those contacts are active. And as a newer platform, the integration ecosystem is thinner than the incumbents. None of these are bugs; they are the natural consequences of being a polished hosted SaaS.
...
On ownership, Hogsend is open source and self-hosted: your data sits in your own Postgres, and there is no per-contact or per-email pricing -- you pay only for infrastructure. It is consumed as a versioned engine package (`@hogsend/engine`), so you scaffold with `pnpm dlx create-hogsend@latest` and upgrade with `pnpm up "@hogsend/*"`. It is a dependency you pin, not a fork you maintain.

...
<Callout type="info">
The `@hogsend/*` packages are not on npm yet (everything is at `0.0.1` while the publish pipeline finishes) -- they're landing imminently. Until then they're consumed from this monorepo, and the scaffold's verification harness resolves them from local `pnpm pack` tarballs. The commands here are the intended, final shape. See [Getting Started](/docs/getting-started) for the current local-dev path.
</Callout>

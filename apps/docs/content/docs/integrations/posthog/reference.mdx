...
|---|---|---|
| Event capture (`getAnalytics()?.capture()`) | `POSTHOG_API_KEY` | Silent no-op (`getAnalytics()` returns `undefined`) |
...
PostHog **feature flags** are not in this table because they are not on the
engine's analytics wire. `AnalyticsProvider` is deliberately narrow — capture
and person properties — so read flags from the PostHog SDK directly, or use
Hogsend's own [feature flags](/docs/flags).

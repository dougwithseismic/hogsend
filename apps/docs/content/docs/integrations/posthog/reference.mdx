...
```json title="https://hogsend.com/.well-known/hogsend-posthog-client.json"
{
  ...
  "scope": "person:read person:write project:read hog_function:write"
}
```
...
The connect flow requests four scopes, and team-level access (the narrowest consent PostHog offers — the grant is scoped to one project's team):
...
| `project:read` | One-shot `GET /api/projects/@current/` project discovery |
...
- Access tokens (`pha_…`) live ~10 hours; the engine refreshes about **60 seconds before expiry**, using the token endpoint discovered at connect time.
...
| Events in (PostHog triggers journeys) | The webhook destination + `POSTHOG_WEBHOOK_SECRET` | No PostHog-triggered journeys; the [data plane](/docs/data-api/events) still ingests everything else |
...
| Loop provisioning | A credential with `hog_function:write` + `POSTHOG_WEBHOOK_SECRET` + a reachable `API_PUBLIC_URL` | Refused with one of the `409` codes above |

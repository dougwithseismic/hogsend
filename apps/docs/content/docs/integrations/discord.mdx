...
**In** — three Discord Gateway dispatches become `discord.*` events out of the box, and a fourth (presence) is opt-in. Each runs through the same [ingestion pipeline](/docs/data-api/events) as PostHog and the REST API: stored in `user_events`, routed to matching journeys, exit-checked, and upserted onto a contact.

- `discord.message_sent`, `discord.reaction_added`, `discord.member_joined` — see [Events](#events) for the full mapping. `discord.presence_active` needs the `GUILD_PRESENCES` intent, which is [not requested by default](#presence-is-opt-in).
...
Leave the third toggle, **Presence**, off. `createDiscordGatewayWorker` does not put `GUILD_PRESENCES` in its default bitfield, so enabling it in the portal alone changes nothing: see [Presence is opt-in](#presence-is-opt-in).
...
The first three arrive out of the box. `PRESENCE_UPDATE` does not: see [Presence is opt-in](#presence-is-opt-in).
...
The transform arm is untouched, so the intent is the only switch. To opt back in, toggle **Presence** in the portal **and** pass an explicit bitfield. The shape depends on which host runs the socket. On [the standalone host](#legacy-the-standalone-worker-path) (`CONNECTOR_RUNTIME_HOST=standalone`), the worker keeps the HTTP ingress hop, so it takes a real `apiPublicUrl` + `ingressSecret` and no `poster`:

```ts title="standalone host (CONNECTOR_RUNTIME_HOST=standalone)"
createDiscordGatewayWorker({
  botToken,
  apiPublicUrl,
  ingressSecret,
...
```ts title="src/worker.ts (worker-hosted, presence on)"

---
title: MCP server
description: "The @hogsend/mcp server exposes a running Hogsend instance to MCP clients (Claude Desktop, Cursor, and claude.ai connectors) with five tools (manage_blueprint, hogsend_report, get_referral_report, get_referral_tree, send_test_email), the blueprint authoring-guide resource, and the find_and_fix_bottleneck prompt. It runs over stdio via npx or as a consumer-mounted Streamable HTTP route at POST /v1/mcp, and every call is admin-gated with the operator's own full-admin key."
---
...
| `get_referral_report` (tool) | Read-only referral leaderboard: who is referring, and the revenue their tree produced under the requested `model` / `window` / `depth` / `weights`. Every monetary field is a list of `{ currency, value }`; values are never converted between currencies. |
| `get_referral_tree` (tool) | Read-only drill-in for one referrer: their descendants to `depth` plus their touch log, rejected touches and reasons included. A ledger view: no window, no weights. |

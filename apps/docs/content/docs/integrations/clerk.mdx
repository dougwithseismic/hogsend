...
<Callout type="info">
  Want to customize the mapping? Author your own `defineWebhookSource()` with `id: "clerk"` in your app's `src/webhook-sources/`. A consumer source with the same id **overrides** the built-in preset — your transform wins. See [Events &amp; Ingestion](/docs/guides/events) for the `defineWebhookSource()` API.
</Callout>

...
Telegram's member-directed actions (`sendMessage`, `dm`) are **preference-gated automatically**. When the engine can resolve the recipient to a contact, the send is skipped if that contact has globally unsubscribed (`unsubscribedAll`) or opted out of the auto-registered **`telegram` channel** list. Instead of a `SendResult`, the action returns a typed `ConnectorActionSkipped` (`{ skipped: true, reason: "unsubscribed_all" | "channel_unsubscribed", connectorId, action }`) — no Bot API call is made. Narrow the union with the `isConnectorActionSkipped` guard exported from `@hogsend/engine`:
...
const res = await sendConnectorAction({
  connectorId: "telegram",
  action: "dm",
  args: { to: user.email, text: "New feature just shipped 🚀" },
});
if (isConnectorActionSkipped(res)) return; // recipient opted out — nothing sent
```

When **no** contact resolves (a raw `chatId`, a group chat, an unlinked id), there is no preference surface, so the send proceeds — gating never blocks an ops message. To opt a contact out of Telegram specifically, unsubscribe them from the channel: `POST /v1/lists/telegram/unsubscribe`. The skip verdict is recorded in the durable journal, so it is replay-stable — a preference flip between a run and its replay can't change what already happened.

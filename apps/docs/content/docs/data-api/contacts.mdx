...
| `anonymousId` | `string` | No | Your analytics provider's anonymous id. Not a third identity — `email` or `userId` is still required. Sent before any `userId` exists, the contact key *becomes* this id (zero-merge stitching); a later `userId` flips the key and fires one `$create_alias`. See [Identity](/docs/data-api/identity). |

...
`list` / `get` only ever expose the display `secretPrefix` (e.g. `whsec_AbCd`); the full secret is recoverable only from `create` and `rotateSecret`. See [Outbound webhooks](/docs/data-api/webhooks) for the event catalog, envelope, and delivery semantics.

...
`hs.links.*` mints and manages tracked links (the surface behind the Studio **Links** view and the `/l/:slug` short URLs), including their print-ready QR codes.

<Callout type="warn">
  Like `hs.webhooks.*`, `hs.links.*` targets the **admin plane** (`/v1/admin/links`) and requires a **full-admin** `apiKey`, not the `ingest` data key. Use an admin-scoped client (the `admin` client above).
</Callout>

```ts
const link = await admin.links.create({
  url: "https://example.com/launch",
  slug: "spring-mailer", // idempotent: re-running returns the same link
  // Or, for a slugless link (a standalone QR), dedupe re-runs with a key:
  // idempotencyKey: "spring-poster",
}); // → { id, vanityUrl: ".../l/spring-mailer", existing, ... }

const png = await admin.links.qr(link.id, { format: "png", size: 1024 }); // Uint8Array
const svg = await admin.links.qr(link.id, { format: "svg" }); // string
const qrUrl = admin.links.qrUrl(link.id, { format: "svg" }); // authless URL, no request

// Re-point a printed QR: the code encodes the durable id, not the URL.
await admin.links.update(link.id, { originalUrl: "https://example.com/v2" });

await admin.links.get(link.id); // → LinkDetail (flat link + recent clicks)
await admin.links.list({ limit: 50, type: "public" }); // → { links, total, limit, offset }
await admin.links.archive(link.id); // soft-delete; the short URL keeps redirecting
```

| Method | Returns | Notes |
|---|---|---|
| `create(input)` | `CreatedLink` | Idempotent by `slug` or `idempotencyKey`; `existing: true` on a compatible re-mint. `source` defaults to `"api"` |
| `get(id)` | `LinkDetail` | Flat link plus recent clicks and per-destination stats |
| `list(opts?)` | `LinkList` | `{ links, total, limit, offset }`; filter by `type` / `includeArchived` / `hasQr` |
| `update(id, input)` | `Link` | Re-point `originalUrl` (a printed QR follows), rename, or re-slug |
| `archive(id)` | `Link` | Soft-delete; the short URL keeps redirecting |
| `qr(id, opts?)` | `Uint8Array` \| `string` | PNG bytes, or an SVG string when `format: "svg"` |
| `qrUrl(id, opts?)` | `string` | Builds the QR URL locally, no request. Attach admin auth yourself |

`create` is safe to re-run: a slug re-mint with the same destination and type returns the existing link instead of a `409`, and a slugless mint deduped on `idempotencyKey` does the same. A different destination, a different type, a slug held by an archived link, or a reused `idempotencyKey` with a different URL is a `409`.

...
Hogsend emits a fixed catalog of **12 event types**. When you create an endpoint you subscribe it to one or more of these — only the events you list are delivered.
...
| `email.delivered` | The provider confirms delivery to the inbox |
| `email.opened` | The open pixel fires for a sent email |
| `email.clicked` | A tracked link in an email is clicked |
...
```json title="email.clicked — emailSendId / resendId / templateKey / to / linkUrl / linkId"
{
  "id": "msg_9a2b...",
  "type": "email.clicked",
...
- **`email.sent`** carries `{ emailSendId, resendId, templateKey, to, userId, category, journeyStateId, subject, sentAt }`.
- **`email.delivered` / `email.opened`** carry the common email shape `{ emailSendId, resendId, templateKey, userId, to, at }`.
...
Body fields: `url` (required, must be a valid URL), `eventTypes` (required, at least one catalog event), `description` (optional, ≤500 chars), `disabled` (optional — create the endpoint disabled).
...
Every field is optional: `url`, `eventTypes` (replaces the set; at least one), `description` (pass `null` to clear it), `disabled`. Returns the updated endpoint (no `secret`).

...
Hogsend emits a fixed catalog of **27 event types**. When you create an endpoint you subscribe it to one or more of these — only the events you list are delivered.
...
| `voice.call_started` | An AI voice call started ringing |
| `voice.call_ended` | A voice call ended normally — carries the outcome |
| `voice.no_answer` | A voice call was not answered |
| `voice.voicemail` | A voice call reached voicemail |
| `voice.failed` | A voice call failed (telephony/pipeline fault) |
| `voice.data_collected` | Structured data was extracted from a call — carries `data` |
...
- **`voice.call_started`** carries `{ voiceCallId, providerCallId, agentKey, to, userId, journeyStateId, startedAt }`; **`voice.call_ended` / `voice.no_answer` / `voice.voicemail` / `voice.failed`** carry `{ voiceCallId, agentKey, userId, to, at, reason, durationSec }` (`voice.failed` adds `errorCode`/`errorReason`); **`voice.data_collected`** carries `{ voiceCallId, agentKey, userId, to, at, data }`.

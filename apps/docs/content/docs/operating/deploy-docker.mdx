...
- **A Hatchet token** — see below; this is [Step 1](/docs/getting-started/hatchet).
- **A real `BETTER_AUTH_SECRET`** — `pnpm gen:secret` (`openssl rand -base64 32`).
- **A `RESEND_API_KEY`** — from the [Resend dashboard](https://resend.com/api-keys).
...
Because the app needs a Hatchet token to boot but the token can only be minted after the engine is running, bring-up is **ordered**: infra → token → app. There is no auto-mint.
...
See [Acquire a Hatchet](/docs/getting-started/hatchet) for the Cloud and bring-your-own alternatives — paste a Hatchet Cloud token instead and set `HATCHET_CLIENT_TLS_STRATEGY=tls`.

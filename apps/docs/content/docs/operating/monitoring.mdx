...
description: "Health checks, Slack/webhook/email alerts, dead letter queue, and audit logs — operational visibility out of the box"
---
...
For deeper metrics on journeys, emails, and events, see [Metrics & Analytics](/docs/operating/metrics).
...
1. **Check health** -- `GET /v1/health` -- is the database or Redis down? Is `status` `migration_pending` (a track behind)? Check `schema.engine.inSync` and `schema.client.inSync`.

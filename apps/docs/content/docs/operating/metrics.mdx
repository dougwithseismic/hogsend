...
Use this to compare template performance side-by-side and identify templates that need attention (low open rates, high bounce rates, etc.). Accepts a `from`/`to` window (on `createdAt`) and `includeUntemplated=true` to fold raw sends (no `templateKey`) into a `"(none)"` bucket. `openRate` divides by `delivered`, falling back to `sent` when no deliveries are recorded yet; `clickToDeliveryRate` (clicks ÷ delivered) is the most robust headline when open tracking is partially blocked.

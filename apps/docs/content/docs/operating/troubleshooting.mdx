...
If you used the Go `hogsend init` deployment CLI (see [CLI](/docs/cli)), it set both values when it created the PostHog webhook. If they got out of sync (e.g., you regenerated secrets), update both sides to match.
...
A common false alarm is bootstrapping with `pnpm db:push` or a seed: those create tables **without** writing a migration ledger row, so the health probe reports `inSync: false` even though the tables exist. Always bring up databases with `pnpm db:migrate` from day one. See [Deployment](/docs/operating/deployment#two-track-migrations).
...
These cover the Go `hogsend` deployment/admin CLI (see [CLI](/docs/cli)) — `init` configures an existing Railway-template deploy, it does not scaffold a project (project creation is `pnpm dlx create-hogsend`). This is distinct from the Node `@hogsend/cli` `eject` tool covered in [Upgrading & Customizing](/docs/operating/upgrading).

...
**Fix:** Add event matchers in the PostHog destination. Without matchers, PostHog sends every event. Add a matcher for each event your journeys care about (e.g., `user_signed_up`, `trial_started`). See [Manual Webhook Setup](/docs/integrations/posthog/manual-setup#step-4-add-event-matchers-critical) for the full walkthrough.

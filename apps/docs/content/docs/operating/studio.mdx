...
<StudioDemoCallout placement="docs-studio" />

![Hogsend Studio — the Overview dashboard, showing headline delivery and engagement metrics](/images/studio/02-overview-dashboard.png)
...
| **Events** | Every event ingested into the pipeline — who it's from, its source, and its properties — filterable by event, source, user, and time range. |
...
| **Links** | First-party tracked short links minted outside email — label, type (public/personal), live click count, and archive when done. |
| **Campaigns** | One-off broadcasts to a list or bucket — status, audience, scheduled time, and live send progress; cancel one still in flight. |
| **Journeys** | Enrollment + completion funnels per journey, with an enable/disable toggle. |
| **Buckets** | Real-time, code-defined audience membership — current size, enter/leave trends, and which journeys each bucket feeds. |
...
| **Integrations** | Code-registered connectors and destinations — webhook URLs and verify secrets; connect via the CLI, then observe here. |
| **Setup** | A configuration checklist plus sending-domain verification, so you know when the instance is production-ready. |
...
**A. CLI (locally or on the host).** Run the shell-gated [`hogsend studio admin create`](/docs/cli/studio) on a host that holds `DATABASE_URL` + `BETTER_AUTH_SECRET`. It talks **directly to the database** (no HTTP, no running API) and writes the password through Better Auth (scrypt) — never raw SQL, never plaintext at rest.
...
When you can't get in at all — no admin yet, or a forgotten password with no email provider — the [`hogsend studio admin`](/docs/cli/studio) commands above are the guaranteed recovery path. They are gated only by holding `DATABASE_URL` + `BETTER_AUTH_SECRET` and shell access, so they work even when the web is unreachable.

...
description: "API key management — quick-start legacy key or scoped database-backed keys for production"
---

Hogsend API keys are bearer tokens in the `Authorization` header. They guard two surfaces: the **admin/ops plane** (`/v1/admin/*`, this page's focus) and the public **data plane** (`/v1/contacts`, `/v1/events`, `/v1/emails`, `/v1/lists` — see the [Data API](/docs/data-api/authentication)). Hogsend supports two authentication modes — a legacy environment-variable key for quick setup, and database-backed keys with scoped permissions for production use.

...
<Callout type="info">
  The `@hogsend/*` packages publish from the changesets pipeline and are landing on npm imminently. Until then they resolve from this monorepo (the harness uses local `pnpm pack` tarballs). The upgrade commands below are the intended path either way.
</Callout>
...
Because you register content in **your own** files (not by editing a shared engine index), there is no merge conflict on upgrade. See the [guides](/docs/guides) for authoring content against these seams.
...
When you must rewrite internals — or a patch will not stop conflicting — eject the package with the Node `@hogsend/cli` (not the Go `hogsend` deployment CLI documented under [CLI](/docs/cli)):

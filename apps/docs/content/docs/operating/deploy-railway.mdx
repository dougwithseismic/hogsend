...
<Callout type="info">
  A one-click [Railway template](https://railway.com/deploy/LxSCyR) provisions the topology for you. The walkthrough below is the manual path if you'd rather wire it up yourself.
</Callout>
...
Once it is running, **mint an API token from its dashboard** and set it as `HATCHET_CLIENT_TOKEN` in both Hogsend services — this is [Step 1](/docs/getting-started/hatchet), the same bring-your-own-token contract as every other target. There is no auto-mint.
...
- [ ] [Acquire a Hatchet token](/docs/getting-started/hatchet) and deploy Hatchet-Lite

...
The template links the internal connections — `DATABASE_URL`, `REDIS_URL`, and the Hatchet gRPC address — for you. Three things still need your input after the first deploy:
...
2. **Public URL** — generate a domain on the **api** service, then set `API_PUBLIC_URL` and `BETTER_AUTH_URL` to it (e.g. `https://api.yourapp.com`). These build the unsubscribe + tracking links inside outgoing email, so a localhost value would leak into real mail.
3. **Hatchet token** — open the **hatchet-lite** dashboard, mint an API token, and set `HATCHET_CLIENT_TOKEN` on both the api and worker, then redeploy. See [Get a Hatchet token](/docs/getting-started/hatchet).

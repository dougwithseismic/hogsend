...
Deploy as a separate Railway service from the Docker image `ghcr.io/hatchet-dev/hatchet/hatchet-lite:v0.84.0` (pinned to a concrete `>= v0.80.0` tag so journey replay-safety's durable `memo` fast path is guaranteed), with its own Postgres instance:
...
Deployed ≠ launched — the domain verification, PostHog event loop
(`hogsend connect posthog`), webhook secrets, and go-live smoke live in the
[launch checklist](/docs/operating/launch-checklist).

## Next steps

<Cards>
  <Card title="Launch checklist" href="/docs/operating/launch-checklist" />

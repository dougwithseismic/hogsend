...
| `EMAIL_DOMAIN` | No | host part of `EMAIL_FROM` (falling back to `RESEND_FROM_EMAIL`) | API, Worker | The sending domain the [domain-status service](/docs/cli/domain) reports on, and the domain [test mode](/docs/operating/test-mode)'s `auto` arming keys on. `create-hogsend --domain` writes it for you. Set it explicitly when you send from a subaddress domain that differs from the one registered at the provider. |
| `HOGSEND_TEST_MODE` | No | `auto` | API, Worker | `auto` \| `true` \| `false`. Controls the [test-mode](/docs/operating/test-mode) send redirect. `auto` redirects only while the active provider supports domains **and** `EMAIL_DOMAIN` is set **and** unverified (fail-open: a provider outage never silently redirects prod mail; no `EMAIL_DOMAIN` ⇒ stays live). `true` always redirects; `false` never does. |
...
Optional — connects to your PostHog instance for event ingestion, person property access, and the outbound engagement stream. The full integration reference (OAuth scopes, credential resolution, degradation) is the [PostHog reference](/docs/integrations/posthog/reference).

| Variable | Required | Default | Process | Description |
|---|---|---|---|---|
| `POSTHOG_API_KEY` | No | — | Worker | PostHog **project** API key (`phc_…`) for event capture and person property writes. Without it, `container.analytics` is a no-op. |
| `POSTHOG_HOST` | No | `https://us.i.posthog.com` | Worker | PostHog ingestion host. EU Cloud: `https://eu.i.posthog.com`. Self-hosted: your instance URL. |
| `POSTHOG_PERSONAL_API_KEY` | No | — | Worker | Fallback **read** credential for person properties (timezone resolution, property conditions) — used when no OAuth credential from [`hogsend connect posthog`](/docs/integrations/posthog/connect) is stored. Without either, reads soft-fail to contact properties. |
| `POSTHOG_PROJECT_ID` | No | discovered | Worker | Skips the one-shot `@current` project discovery on the private API. |
| `POSTHOG_PRIVATE_HOST` | No | derived | Worker | Overrides the private (app) API host — by default derived from `POSTHOG_HOST` by stripping the `.i.` ingestion label. |
...
Powers [`refineContact()`](/docs/guides/refinement). Entirely opt-in: with no provider configured, refinement is inert and returns `{ status: "skipped", reason: "no_provider" }` without throwing, so a deploy with no enrichment is a valid deploy.

**Every lookup costs money.** The engine owns the spend controls rather than the provider: results — hits *and* misses — are cached in the `enrichment_lookups` ledger, and the monthly cap fails closed.

**Install the provider package first — the key alone does nothing.** Run `pnpm add @hogsend/plugin-apollo` in your app, or scaffold with `create-hogsend --with apollo`. It must be a direct dependency of *your* app: the engine lists it under `optionalDependencies`, but your app bundles the engine, so the engine's lazy `import("@hogsend/plugin-apollo")` resolves against your `node_modules`, where an engine-only optional dependency is never linked. Set the key without the package and the boot log says exactly that. The same applies to `@hogsend/plugin-postmark` and `@hogsend/plugin-twilio`.

| Variable | Required | Default | Process | Description |
|---|---|---|---|---|
| `APOLLO_API_KEY` | No | — | API, Worker | Apollo API key. Present, plus `@hogsend/plugin-apollo` installed, builds the preset provider. Absent means no enrichment provider is registered. |
| `ENRICHMENT_PROVIDER` | No | `apollo` | API, Worker | Active provider id resolved from the registry. With exactly one provider registered and none requested, that one wins. An explicitly requested but unregistered id throws at boot. |
| `ENRICHMENT_TTL_DAYS` | No | `90` | API, Worker | How long a ledger row satisfies a lookup before the vendor is asked again. Applies to **misses** too: a "not found" is a paid result and is negatively cached, so you do not pay for the same dead email twice. The expiry is materialised on write, so changing this later does not retroactively expire or extend existing rows. |
| `ENRICHMENT_MONTHLY_LOOKUPS` | No | `0` (uncapped) | API, Worker | Hard monthly cap on vendor lookups. A positive cap **fails closed**: once exhausted, `refineContact()` returns `{ status: "skipped", reason: "budget_exceeded" }` rather than spending. Counts vendor **calls**, not ledger rows, so a `force: true` refresh loop cannot spend past it. |

<Callout type="warn">
**The default is uncapped.** That is the right default for a feature that is off until you configure it, and the wrong thing to leave in production by accident. Set `ENRICHMENT_MONTHLY_LOOKUPS` deliberately in every environment that has an API key.
...
ENABLED_JOURNEYS=*
ENRICHMENT_MONTHLY_LOOKUPS=1000             # only if APOLLO_API_KEY is set; the default is UNCAPPED
```

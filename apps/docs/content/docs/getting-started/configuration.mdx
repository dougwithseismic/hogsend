...
| `HOGSEND_GRAPH_MANIFEST` | No | `<cwd>/.hogsend/journeys.graph.json` | API | Path to the [journey graph manifest](/docs/concepts/journey-graphs) written by `hogsend journeys graph --all`. The admin graph route (`GET /v1/admin/journeys/{id}/graph`) serves the rich control-flow graph from it; without a manifest it falls back to a metadata-only skeleton. Set it when the API's working directory differs from where the manifest was generated (the Docker image pins it to `/app/.hogsend/journeys.graph.json`). |

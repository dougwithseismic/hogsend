...
`pnpm bootstrap` does this copy for you — generating a real `BETTER_AUTH_SECRET` and minting a Hatchet token (see [Installation](/docs/getting-started/installation)). The `.env.example` shipped by the scaffold already points `DATABASE_URL` and `REDIS_URL` at the local Docker Compose ports, so for local development you mostly fill in secrets.
...
[Hatchet](https://hatchet.run) handles durable task execution — journey orchestration, email sends, and background jobs. You'll need a token before first boot — see [Get a Hatchet token](/docs/getting-started/hatchet). These three values are the whole connection.

| Variable | Required | Default | Process | Description |
|---|---|---|---|---|
| `HATCHET_CLIENT_TOKEN` | **Yes** | — | API, Worker | Bearer JWT for connecting to the Hatchet engine. The SDK hard-throws if empty. Copy from the Hatchet dashboard (`localhost:8888` locally) or paste a Hatchet Cloud token. See [Get a Hatchet token](/docs/getting-started/hatchet). |
...
| `HATCHET_CLIENT_TLS_STRATEGY` | No | `tls` | API, Worker | TLS strategy for the gRPC connection: `none` \| `tls` \| `mtls`. **Secure by default.** Override to `none` for local and internal-network hatchet-lite; keep `tls` for Hatchet Cloud and the public internet. |
...
| `ADMIN_API_KEY` | No | — | API | API key for `/v1/admin/*` endpoints and the `hogsend` CLI's admin commands. Without it, admin endpoints are inaccessible. |

<Callout type="warn">
  `ADMIN_API_KEY` gates all admin operations — viewing contacts, managing journeys, metrics, and the CLI's `contacts` / `journeys` commands. Generate a strong key: `openssl rand -hex 32`.
</Callout>

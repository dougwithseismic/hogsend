...
| `STUDIO_SETUP_TOKEN` | No | auto-generated | API | Gate for the Studio first-admin create, closing the first-run land-grab on a public deploy. Set it and paste the value into the create-admin form. If unset, the engine auto-generates one and prints it to the server log **once** on first boot (never returned over HTTP). Irrelevant once an admin exists (signup closes). See [Operating → Studio](/docs/operating/studio#the-setup-token-closes-the-first-run-land-grab). |
...
| Variable | Required | Default | Process | Description |
|---|---|---|---|---|
| `RESEND_API_KEY` | **Yes** | — | Worker | Resend API key for sending emails. From the [Resend dashboard](https://resend.com/api-keys). Starts with `re_`. The scaffold ships a placeholder — set a real key before sending. |

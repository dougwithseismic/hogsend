...
This connects your PostHog instance to a **running** Hogsend API. Make sure you've completed [Installation](/docs/getting-started/installation) and `GET /v1/health` returns healthy. PostHog forwards events to your API's `POST /v1/webhooks/posthog` endpoint, which feeds the same ingestion pipeline that triggers journeys.

<Callout type="warn">
  PostHog's servers must be able to reach your API over the internet. A locally-running app on `http://localhost:3002` is **not** reachable by PostHog Cloud — deploy first (see [Deployment](/docs/operating/deployment)), or use a tunnel (ngrok, Cloudflare Tunnel) to expose your local API while testing.
</Callout>
...
| **Outbound** | Hogsend → PostHog | `POSTHOG_API_KEY` | The engine's _own_ analytics capture — including email **open** and **link click** events it generates — flows back into your PostHog project. |

<Callout type="warn">
  **Email open/click tracking does NOT automatically show up in PostHog.** Hogsend always records opens and clicks in its own database, but it only mirrors them into PostHog when **`POSTHOG_API_KEY`** is set (outbound). That key is separate from `POSTHOG_WEBHOOK_SECRET` (inbound) — setting the webhook secret on this page gets events _into_ Hogsend, but it does nothing for getting Hogsend's engagement events _into_ PostHog. If you want opens/clicks in PostHog, set `POSTHOG_API_KEY` too.
</Callout>
...
Both sides must match. PostHog sends the `x-posthog-webhook-secret` header; Hogsend compares it against the `POSTHOG_WEBHOOK_SECRET` env var on your API. If they don't match, Hogsend rejects the request with **`401 Invalid webhook secret`** and the event never enters the pipeline. (If `POSTHOG_WEBHOOK_SECRET` is unset, the source is treated as open and accepts unauthenticated requests — set it in any internet-reachable deployment.)
  ...
  "events": [
    {
      ...
      "properties": { "email": "alice@example.com", "_posthogEventId": "…" },
...
Check `userId` (mapped from `distinct_id`) and `properties.email` are populated — that's the proof the contact has an address and the journey can email them. You can also narrow to a single contact with `&userId=user_abc123`.
...
- Is `POSTHOG_WEBHOOK_SECRET` set in your Hogsend environment, and does the `x-posthog-webhook-secret` header value match it exactly? A mismatch returns `401` and the event is dropped — check PostHog's History tab for `401`s.

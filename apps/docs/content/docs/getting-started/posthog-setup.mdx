...
<Callout type="info">
  If you deployed via the Railway template and ran the Go `hogsend init` CLI, it already created this webhook destination for you (URL, auth header, and event matchers). This guide is the manual equivalent — read it to understand or customize what `init` set up. (`hogsend init` is the Go deployment CLI, distinct from `create-hogsend`, which scaffolds your app.)
</Callout>

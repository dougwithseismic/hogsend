...
1. **PostHog** — via a webhook source. PostHog is the standard event source (the scaffold ships one ready to register), but it is **not required to boot** — a Hogsend instance runs fine without it.
2. **Your own app** — call the public data plane directly: `POST /v1/events` (with an ingest-scoped `HOGSEND_API_KEY`) or the [`@hogsend/client`](/docs/data-api/client-sdk) SDK. This is the first-party front door for events from your signup handler, billing webhook, or a cron.
3. **Other systems** (Stripe, Clerk, Supabase, Segment, …) — via built-in [integration presets](/docs/integrations) or your own custom webhook sources.
4. **Hogsend itself** — lifecycle events like `journey:completed` and `journey:failed` (and bucket transitions) are emitted automatically and can trigger other journeys.

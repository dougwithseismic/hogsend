...
The wait is **forward-looking** — only events emitted _after_ the wait begins count. Use [`ctx.history.hasEvent()`](#ctxhistoryhasevent) to check whether something already happened. `timeout` is required and capped at the journey's execution limit (`720h` / 30 days).

...
```ts
journey(opts:
  | { userId: string; journey: DefinedJourney }
...
Hand over the **journey**, not its id:
...
`journeyId: "activation-welcome"` still works, but nothing checks it. Misspell it
— or rename the journey six months later — and you get
`{ completed: false, entryCount: 0 }` forever, which is indistinguishable from a
user who genuinely never finished. On a call whose entire job is deciding whether
to route someone into another flow, that is a wrong answer that looks like a
right one. `journey: activationWelcome` cannot be misspelled (an unknown symbol
is a compile error) and a rename follows the symbol.

Keep `journeyId` for ids that only exist at **runtime** — read from config, a
Blueprint, a Studio-authored journey, or an agent's input. That is the case the
type system cannot serve, not a second spelling of the same thing. Pass exactly
one of the two.
...
email(opts: {
  email: string;
  // a key of your template registry, not any string
...
`template` is narrowed to the same union `sendEmail({ template })` enforces, so
a typo is a compile error instead of a lookup that answers "never sent"
forever. Before you register your first template the union is empty and the
type widens back to `string`.

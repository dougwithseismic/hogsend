...
<Callout type="info">
  ...
  including [`hogsend connect posthog`](/docs/integrations/posthog/connect),
  which wires person reads and the inbound webhook in one command — see
  [the PostHog integration](/docs/integrations/posthog); for fanning Hogsend's
...
|---|---|---|
| Event capture (`getAnalytics().capture`, destinations) | `POSTHOG_API_KEY` | Silent no-op |
...
[webhook source](/docs/integrations/posthog/manual-setup). Non-interactive:
...
deployed, [`hogsend connect posthog`](/docs/integrations/posthog/connect)
...
4. **It round-trips.** Events forwarded back from PostHog (the
   [webhook source](/docs/integrations/posthog/manual-setup)) under that key
...
`setPersonProperties` on the provider puts contact truth onto the analytics
person — plan, role, lifecycle stage — using only the project key. Reach the
active provider with `getAnalytics()`, which is vendor-neutral and returns
`undefined` when no provider is configured:
...
destination](/docs/data-api/destinations) propagates contact changes

...
Mutating a bucket that is not `kind: "manual"` is refused with a 409: its membership is owned by its criteria, and an explicit write would be reverted by the next evaluation.

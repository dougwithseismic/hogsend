...
### Trial expiring soon — property inclusion + exclusion

Pure property predicates: on a trial, ≤3 days left, not already converted. No rolling window, so this bucket is real-time only — never swept. `reentry: "once"` means the join fires a single time per user.

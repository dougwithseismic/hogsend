...
`account.unlinked` and `account.link_failed` work the same way. The properties
are scalars: `provider`, `providerUserId`, `username`, `method`, `relink`,
`version`, `state`.
...
replaces the other. A failed link never creates a contact, so a journey sees
`account.link_failed` only for a contact that already exists.
</Callout>

...
<Callout type="warn" title="A backfill enrols journeys, one per row">
An imported link is a real link, so it reaches the journey plane like any
other. Backfill 1000 rows and your `account.linked` journey runs 1000 times —
if it sends a welcome email, that is 1000 emails. Imported rows are stamped
`method: "import"`, so exclude them where you do not want them:

```ts
where: (b) => b.prop("method").neq("import")
```

Re-running the same import is safe: an unchanged pair transitions nothing, so
it emits nothing and enrols nothing.
</Callout>

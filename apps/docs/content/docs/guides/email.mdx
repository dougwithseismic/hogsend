...
## In-email actions (semantic links)

A link can carry an answer, not just a destination. `EmailAction` (from `@hogsend/email`) renders an anchor whose click fires a real event — `nps.submitted { score: 9 }`, `checkin.answered { answer: "yes" }` — through the full ingest pipeline: journeys react via `ctx.waitForEvent`, the answer lands in `user_events`, and destinations receive it as `email.action`. First answer per send wins; scanner click-bursts are judged in full (after a ~30s window) before anything is recorded.
...
See the [Semantic links guide](/docs/guides/semantic-links) for the full pattern — answer semantics, validation rules, and the journey side.

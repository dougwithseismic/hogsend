...
Implement the `EmailProvider` interface for your provider and pass it under the grouped `email` option (alongside your `templates`) — that's the whole swap:
...
//   event.messageId   -> provider message id

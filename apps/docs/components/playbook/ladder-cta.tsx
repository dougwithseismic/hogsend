...
import { TrackPlayCta } from "./play-tracking";
...
/** The three-rung ladder block at the bottom of every play. Each rung click
 * fires `docs.play_cta_clicked` on both analytics legs (see play-tracking). */
export function LadderCta({ slug }: { slug: string }): JSX.Element {
  return (
    <div className="grid gap-4 md:grid-cols-3">
      {RUNGS.map((rung) => (
        <TrackPlayCta key={rung.href} rung={rung.rung} slug={slug}>
          <Link
            href={rung.href}

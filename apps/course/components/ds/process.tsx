...
export function ProcessSteps({
  ...
  return (
    ...
    >
      {/* lg:self-start is load-bearing: grid items stretch to the row's full
          height by default, leaving sticky no room to travel. */}
      <div className="lg:sticky lg:top-28 lg:self-start">
        {eyebrow ? <Eyebrow className="mb-4">{eyebrow}</Eyebrow> : null}
        {title ? <h2 className={cn(H2_CLASS, "max-w-xl")}>{title}</h2> : null}
        {subtitle ? (

 ...
 */
/**
 * One run's mutable state, threaded through every step. The fields set inside a
 * step (`tenantDsn`, `refs`, `secrets`, …) are optional because they do not
 * exist until the step that produces them runs; the linear driver guarantees a
 * consumer never runs before its producer.
 */
interface RunState {
  stackId: string;
  context: ProvisionContext;
  /** The freshest stack row; every `patchStack`/`transition` re-seats it. */
  stack: StackRow;
  tenantDsn?: string;
  poolMax?: number;
  hatchetToken?: string;
  ...
  sesTenant?: Awaited<ReturnType<typeof provisionSesTenant>>;
  steps: ProvisionStepReport[];
  /**
   * Set by `start` when the loaded stack is already `running`: the driver reads
   * it after each step and returns the running result without a finish
   * transition or a welcome send.
   */
  shortCircuit?: boolean;
  /** Set only on the run that actually reaches `finish`. */
  welcome?: { stack: StackRow; context: ProvisionContext } | null;
}

type StepFn = (state: RunState, deps: ProvisionDeps) => Promise<void>;

/**
 * The audit row a step writes. Same payload the pipeline has always written —
 * lifted to a module function so each step unit can call it directly.
 */
...
// ---- start ----------------------------------------------------------------
// `provisioning` is legal from `requested` AND from `error` — the retry edge —
// so one call covers a first run and a dashboard resume. A stack ALREADY in
// `provisioning` is not an error either: that is what a replayed durable task
// looks like.
async function stepStart(state: RunState, deps: ProvisionDeps): Promise<void> {
  const { context } = state;
  if (state.stack.status === "running") {
    // Nothing to do, and nothing to say: a second enqueue for a finished stack
    // must not walk it back through provisioning. The driver returns the
    // running result on this flag without pushing a step report.
    state.shortCircuit = true;
    return;
  }
  if (state.stack.status !== "provisioning") {
    state.stack = await deps.stackService.transition({
      stackId: state.stackId,
      to: "provisioning",
      actor: PROVISIONER_ACTOR,
      detail: { step: "start", from: state.stack.status },
    });
  }
  state.steps.push({
    step: "start",
    skipped: state.stack.status === "provisioning",
  });
  await auditStep(deps, state.stackId, "start", context.organizationId, {
    from: context.stack.status,
  });
}

// ---- ensure-tenant-db ------------------------------------------------------
async function stepEnsureTenantDb(
  state: RunState,
  deps: ProvisionDeps,
): Promise<void> {
  const { context } = state;
  const dbName = state.stack.dbName;
  if (!dbName) throw new Error(`Stack ${state.stackId} carries no db_name`);

  state.poolMax = 3;
  if (state.stack.dbDsnEncrypted) {
    // The artifact of a previous run: reuse it verbatim. Re-creating would
    // report `alreadyExists` with no DSN, and rotating would break a stack that
    // is already running on this credential.
    state.tenantDsn = decryptSecretPayload<string>(state.stack.dbDsnEncrypted);
    state.steps.push({ step: "ensure-tenant-db", skipped: true });
    await auditStep(
      deps,
      state.stackId,
      "ensure-tenant-db",
    ...
    if (!context.cellDsn) {
      throw new Error(
        `Organization ${context.organizationId} has no cell; dedicated-topology provisioning is not implemented (PRD 04 task 5)`,
      );
    }
    ...
    if (created.alreadyExists) {
      // The database is there but its password is unknowable — a run that died
      // between CREATE and persist. Rotating is the ONLY way back, and is safe
      // precisely because `set-env` re-pushes the DSN below.
      const reset = await deps.tenantDb.resetCredentials({
        cellDsn: context.cellDsn,
        dbName,
      });
      state.tenantDsn = reset.dsn;
      state.poolMax = reset.poolMax;
    } else {
      state.tenantDsn = created.dsn;
      state.poolMax = created.poolMax;
    }
    state.stack = await patchStack(deps.db, state.stackId, {
      dbDsnEncrypted: encryptSecretPayload(state.tenantDsn),
    });
    state.steps.push({ step: "ensure-tenant-db", skipped: false });
    await auditStep(
      deps,
      state.stackId,
      "ensure-tenant-db",
      context.organizationId,
      { dbName, rotated: created.alreadyExists, poolMax: state.poolMax },
    );
  }
}

// ---- mint-hatchet ----------------------------------------------------------
async function stepMintHatchet(
  state: RunState,
  deps: ProvisionDeps,
): Promise<void> {
  const { context } = state;
  const namespace = state.stack.hatchetNamespace ?? state.stackId;
  state.namespace = namespace;
  if (!context.cellHatchetUrl) {
    throw new Error(
      `Organization ${context.organizationId} has no cell Hatchet url; cannot mint a tenant token`,
    );
  }
  ...
  state.hatchetHostPort = addresses.hostPort;
  if (state.stack.hatchetTokenEncrypted) {
    state.hatchetToken = decryptSecretPayload<string>(
      state.stack.hatchetTokenEncrypted,
    );
    state.steps.push({ step: "mint-hatchet", skipped: true });
    await auditStep(
      deps,
      state.stackId,
      "mint-hatchet",
      context.organizationId,
      { namespace, reused: true },
    );
  } else {
    ...
    state.hatchetToken = minted.token;
    state.stack = await patchStack(deps.db, state.stackId, {
      hatchetTokenEncrypted: encryptSecretPayload(minted.token),
      hatchetNamespace: namespace,
    });
    state.steps.push({ step: "mint-hatchet", skipped: false });
    await auditStep(
      deps,
      state.stackId,
      "mint-hatchet",
      context.organizationId,
      { namespace, createdTenant: minted.createdTenant },
    );
  }
}

// ---- substrate-provision ---------------------------------------------------
async function stepSubstrateProvision(
  state: RunState,
  deps: ProvisionDeps,
): Promise<void> {
  const { context } = state;
  const engineVersion = state.stack.engineVersion ?? deps.defaultEngineVersion;
  state.engineVersion = engineVersion;
  let refs = readStackRefs(state.stack);
  if (refs) {
    state.refs = refs;
    state.steps.push({ step: "substrate-provision", skipped: true });
    await auditStep(
      deps,
      state.stackId,
      "substrate-provision",
      context.organizationId,
      { substrate: refs.substrate, reused: true },
    );
  } else {
    ...
    state.steps.push({ step: "substrate-provision", skipped: false });
    await auditStep(
      deps,
      state.stackId,
      "substrate-provision",
...
//
// The DNS write and the substrate attach are both idempotent, so a re-driven
// run re-derives the same hostname and finds its own record rather than failing
// on it. On any refusal the step is SKIPPED rather than fatal: the instance
// stays on the substrate's own URL, which is exactly where every stack
// provisioned before this step already lives. A hostname is an improvement to
// provisioning, not a precondition for it, and failing a signup over a DNS blip
// would be the wrong trade.
async function stepEnsureHostname(
  state: RunState,
  deps: ProvisionDeps,
): Promise<void> {
  const { context } = state;
  ...
  if (!hostname) {
    state.steps.push({ step: "ensure-hostname", skipped: true });
    await auditStep(
      deps,
      state.stackId,
      "ensure-hostname",
  ...
  const attached = await attachManagedHostname({
    deps,
    organizationId: context.organizationId,
    environmentId: state.stack.environmentId,
    hostname,
    refs: state.refs as StackRefs,
  });
  let certified: { attempts: number; waitedMs: number } | null = null;
  if (attached) {
    // The domain exists; now wait for it to MEAN something. Deliberately before
    // the URL swap: adopting a hostname that cannot serve HTTPS is how the
    // first live provision died three steps later.
    certified = await waitForDomain(deps, state.refs as StackRefs, hostname);
    // Set on EVERY successful pass, not only the one that created the record: a
    // re-driven run finds its own row and must still point the env at the
    // hostname, or `set-env` would freeze the substrate URL for an instance
    // whose name already resolves.
    state.refs = {
      ...(state.refs as StackRefs),
      apiPublicUrl: `https://${hostname}`,
    };
    state.stack = await patchStack(deps.db, state.stackId, {
      substrateRefs: { ...state.refs },
    });
  }
  state.steps.push({
...
// ---- provision-ses ---------------------------------------------------------
// The environment's SES tenancy, and the two credentials the instance needs to
// reach the send relay and to trust what the relay sends back.
//
// FATAL on failure, deliberately: the stack parks at `error` with this step's
// name and the sweep re-drives it, which is the house pattern and costs nothing
// here — no service has been started yet, so nothing is orphaned. A control
// plane with NO AWS credentials never reaches that branch: the factory yields
// the Fake, the converge succeeds, and the tenancy is simply recorded
// unavailable.
async function stepProvisionSes(
  state: RunState,
  deps: ProvisionDeps,
): Promise<void> {
  const { context } = state;
  const sesTenant = await provisionSesTenant(
    { environmentId: state.stack.environmentId, actor: PROVISIONER_ACTOR },
    { db: deps.db },
  );
  state.sesTenant = sesTenant;
  state.steps.push({ step: "provision-ses", skipped: false });
  ...
  await auditStep(
    ...
    context.organizationId,
    {
      ...
      // operator whether to chase credentials, chase AWS support for production
      // access, or do nothing at all.
...
// ---- set-env ---------------------------------------------------------------
async function stepSetEnv(state: RunState, deps: ProvisionDeps): Promise<void> {
  const { context } = state;
  const secrets = await ensureStackSecrets(deps, state.stackId, state.stack);
  state.stack = secrets.stack;
  state.secrets = secrets.secrets;

  const sesTenant = state.sesTenant as Awaited<
    ReturnType<typeof provisionSesTenant>
  >;
  const refs = state.refs as StackRefs;
  const vars = await assembleStackEnv({
    deps,
    context,
    refs,
    tenantDsn: state.tenantDsn as string,
    poolMax: state.poolMax as number,
    hatchetToken: state.hatchetToken as string,
    hatchetHostPort: state.hatchetHostPort as string,
    namespace: state.namespace as string,
  ...
  state.steps.push({ step: "set-env", skipped: false });
  // Audit the KEYS only. The names prove the env was assembled; the values are
  // exactly what must never reach a log.
  await auditStep(deps, state.stackId, "set-env", context.organizationId, {
    keys: Object.keys(vars).sort(),
  });
}

// ---- start-services --------------------------------------------------------
// Worker first, then api. The worker registers its journey tasks with Hatchet
// on boot; bringing the api up first would open the front door on a stack that
// cannot yet execute anything it accepts.
async function stepStartServices(
  state: RunState,
  deps: ProvisionDeps,
): Promise<void> {
  const { context } = state;
  const refs = state.refs as StackRefs;
  const bootImage = qualifyImage(
    defaultImageTag(state.engineVersion as string),
  );
  ...
  state.steps.push({ step: "start-services", skipped: false });
  await auditStep(
    deps,
    state.stackId,
    "start-services",
...
// ---- health-wait -----------------------------------------------------------
async function stepHealthWait(
  state: RunState,
  deps: ProvisionDeps,
): Promise<void> {
  const { context } = state;
  const waited = await waitForHealth(deps, state.refs as StackRefs);
  state.steps.push({ step: "health-wait", skipped: false });
  await auditStep(deps, state.stackId, "health-wait", context.organizationId, {
...
// first-admin bootstrap has already run off the env `set-env` pushed. This step
// signs in as that admin and takes an ingest key out of the instance's own
// admin API — the only place the full key is ever readable.
//
// It runs BEFORE `finish`, and it THROWS on failure, which is the ordering that
// matters: a stack only reaches `running` with `credentialsMinted: true`
// already on the row. A failed mint parks the stack at `error` with this step's
// name, which the provision sweep re-drives on its first condition — recovery,
// not just detection. (The sweep's third condition still covers a `running`
// stack whose refs lost the flag some other way, and T3 alerts on it.)
async function stepMintCredentials(
  state: RunState,
  deps: ProvisionDeps,
): Promise<void> {
  const { context } = state;
  if (!context.ownerEmail) {
    throw new Error(
      `Organization ${context.organizationId} has no member to make the tenant's Studio admin`,
    );
  }
  const minted = await mintTenantCredentials({
    ...
    secrets: state.secrets as StackSecrets,
  });
  state.stack = await patchStack(deps.db, state.stackId, {
    substrateRefs: {
      ...(state.stack.substrateRefs ?? {}),
      credentialsMinted: true,
    },
  });
  state.steps.push({ step: "mint-credentials", skipped: minted.reused });
...
// ---- finish ----------------------------------------------------------------
async function stepFinish(state: RunState, deps: ProvisionDeps): Promise<void> {
  const { context } = state;
  ...
  state.steps.push({ step: "finish", skipped: false });
  await auditStep(deps, state.stackId, "finish", context.organizationId, {
    status: "running",
  });

  // Everything the welcome mail needs, handed to the SEND SITE in the driver —
  // which is outside the try on purpose (see there).
  state.welcome = { stack: state.stack, context };
}

/**
 * The pipeline steps by name. Typed `Record<ProvisionStep, StepFn>` so a step
 * key that is missing or misspelled is a compile error, not a runtime surprise.
 */
const STEP_FNS: Record<ProvisionStep, StepFn> = {
  start: stepStart,
  "ensure-tenant-db": stepEnsureTenantDb,
  "mint-hatchet": stepMintHatchet,
  "substrate-provision": stepSubstrateProvision,
  "ensure-hostname": stepEnsureHostname,
  "provision-ses": stepProvisionSes,
  "set-env": stepSetEnv,
  "start-services": stepStartServices,
  "health-wait": stepHealthWait,
  "mint-credentials": stepMintCredentials,
  finish: stepFinish,
};
...
export async function runProvisionPipeline(
  ...
  // Tracked for the catch: the step whose throw parks the stack, mirroring the
  // `current` the god-function updated at the top of each block.
  ...
  try {
    for (const step of PROVISION_STEPS) {
      current = step;
      await STEP_FNS[step](state, deps);
      if (state.shortCircuit) {
        // `start` found the stack already `running`: return the running result
        // WITHOUT the finish transition and WITHOUT a welcome send.
        return { stackId, status: "running", steps: state.steps };
      }
    }
    result = { stackId, status: "running", steps: state.steps };
  } catch (error) {
  ...
  if (state.welcome) await sendWelcomeEmail(deps, state.welcome);

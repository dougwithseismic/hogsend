...
// Shared, email-safe UI primitives in the Hogsend house style: near-black
// CTAs, an orange accent, monospace for anything technical, generous
// whitespace, hairline borders instead of heavy shadows. Compose these in
// templates instead of repeating Tailwind class soup.
...
export function Eyebrow({ children }: { children: ReactNode }) {
  return (
    <Text className="m-0 mb-3 text-xs font-semibold uppercase tracking-[0.12em] text-orange-600">
      {children}
    </Text>
  );
}
  ...
  return (
    <Section className="my-2">
      {items.map((item, i) => (
        ...
        >
          <span className="mr-2 font-semibold text-orange-500">{marker}</span>
          {item}
        </Text>
      ))}
    </Section>
...
/** Numbered step, with an optional code/detail block underneath. */
export function Step({
  ...
  return (
    <Section className="mb-4">
      <Text className="m-0 text-[15px] font-semibold leading-6 text-zinc-900">
        <span className="mr-2 inline-block rounded-md bg-zinc-900 px-2 py-0.5 text-xs font-bold text-white">
          {index}
        </span>
        {title}
      </Text>
...
/** Single big number with a label — for milestones and digests. */
export function Stat({
  ...
  return (
    <Section className="mb-3">
      ...
      <Text className="m-0 text-[22px] font-bold leading-tight text-zinc-900">
        {value}
      </Text>
      {change && (

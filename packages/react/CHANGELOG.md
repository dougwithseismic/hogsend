...
- a8b195c: Add a two-way `window.dataLayer` bridge so sites already running GTM/GA4 get
...
- 0c1852c: Keep the engine version line uniform: bump every engine-line package (and the
  ...
  publish on one version and the scaffold's `^{{ENGINE_VERSION}}` caret pins stay
  aligned. Generated by `pnpm release-doctor --fix-changeset`.
- ece64d4: Make a late-arriving `userToken` apply in place instead of forcing a

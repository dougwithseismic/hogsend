...
- dbea404: `linked_accounts` table. One row per platform-account link, live or historical, keyed to a contact by a NOT NULL `contact_id` FK. Purely additive: no existing table changes, no column drops, no backfill. **Consumers must run `pnpm db:migrate` on upgrade** — migration `0072_lively_sue_storm` on the ENGINE track.

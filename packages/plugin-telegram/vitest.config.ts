...
export default defineConfig({
  test: {
    ...
    // (the action modules import the engine's `defineConnectorAction`).
    server: {
      deps: {
    ...
    // — mirrors packages/plugin-discord/vitest.config.ts. The Telegram action
    // audience tests never touch a DB, Hatchet, or the network; these only
    // satisfy the boot-time schema.
    env: {
      NODE_ENV: "test",
      PORT: "3002",
      LOG_LEVEL: "error",

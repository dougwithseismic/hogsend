...
import { enrichmentProvidersFromEnv } from "./enrichment-providers-from-env.js";
...
// The guard itself now lives in `load-optional-plugin.ts` and is covered by
// `load-optional-plugin.test.ts` — including the classification this preset
// depends on (absent package vs installed-but-broken). Only the preset's own

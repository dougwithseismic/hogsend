...
- **Service integrations are standalone imports**, not on the context. Email: `sendEmail()` from `@hogsend/engine` (engine `lib/email.ts`). Analytics: `getAnalytics()` from `@hogsend/engine` (engine `lib/analytics-singleton.ts`) returns the active vendor-neutral `AnalyticsProvider` for `capture` / `setPersonProperties` / `getPersonProperties`, or `undefined` when no provider is configured — there are NO ctx.identify/ctx.posthog shims on the context, and the vendor-named `getPostHog()` no longer exists. All functions follow single-object-in, result-object-out pattern

...
- **`JourneyContext`** (engine: `packages/engine/src/journeys/journey-context.ts`) provides only durable execution primitives:
  ...
  - `ctx.variant(key, arms)` — deterministic RECORDED experiment arm for THIS user (pure sha256 over `variant:<journeyId>:<key>:<userId>`, equal split, no RNG/clock). Recorded once per enrollment under `journey_states.context.__variants__` and replayed VERBATIM within that enrollment (even if a deploy changed `arms`); a re-entry re-derives the same arm while the arms array is unchanged. Issues ZERO durable Hatchet calls (journal-invisible). Variant-selected sends need no `ctx.once` wrap and no `idempotencyLabel` of their own; key charset `/^[A-Za-z0-9][A-Za-z0-9_.-]{0,63}$/`. The engine strips `__once__`/`__digest__`/`__throttle__`/`__variants__` from event properties at context seeding (injection fix)
...
When adding a new provider: scaffold a `packages/plugin-<name>` mirroring `plugin-resend`/`plugin-postmark`, implement `createXProvider` via `defineEmailProvider()` (HTML-only send, force native tracking off where possible, normalize webhooks to `EmailEvent`), and add an optional env preset to `emailProvidersFromEnv`. CI publishes brand-new `@hogsend/*` packages fine — the old "first publish must be manual" rule is dead. It was true in the studio era (0.1.0) and has since been disproved repeatedly: `client` + `cli` (0.7.0), bare `hogsend` (0.13.x), `sms` + `plugin-twilio` (0.43.0), `attribution` + `plugin-meta-capi` (0.44.0) were all first-published by CI. `scripts/verify-published.mjs` backstops it post-publish by failing on a 404. Note that a brand-new packument can 404 anonymously for ~15 minutes after a successful publish (read-path lag) — probe the tarball URL rather than declaring a broken release.
...
`ingestEvent()` (engine: `packages/engine/src/lib/ingestion.ts`) is the central event processing function used by both the ingest endpoint and webhook sources. It: (1) stores the event in `userEvents`, (2) pushes to Hatchet (which routes to matching journey tasks), (3) checks exit conditions on active journeys for the user, (4) resolves the contact record, creating one **only when identity is asserted**. Exit conditions are evaluated by matching event name against `exitOn` rules in journey metadata and applying property conditions.

**Contact creation is conditional.** A contact is minted by identity, not by observation: seeing traffic from an unidentified browser is not grounds for a CRM row. `ingestEvent`/`ingestTransformResult` take `allowCreate` (default `true`, so every pre-existing caller is unchanged); with `allowCreate: false` the resolve runs through `resolveContactNoCreate` (`lib/contacts.ts`, exported from `@hogsend/engine`), which returns `{ id: null, resolvedKey }` instead of INSERTing. A refusal is **not** an error and loses no observation: the event still stores in `user_events` under the same canonical key, the Hatchet push and `checkExits` still run, and buckets are still evaluated. What it skips is exactly the work needing a `contacts` row: group association, and (because `conversions.contact_id` / `deals.contact_id` / `funnel_progress.contact_id` are `.notNull()` FKs) conversions, attribution credits, and funnel progress. Refusal is legal only where the canonical key (`external_id ?? anonymous_id ?? id`) is stable, so `resolveContactNoCreate` throws unless the highest-precedence supplied key is `userId` or `anonymousId`; an email-only or `discordId`-only call has no stable refusal key. The built-in refusing callers are `POST /v1/events` on a publishable key with no asserted email/userId, no `value` and no `groups` map; the anon leg of `POST /v1/t/arrive`; and `sendFeedItem` for a pure-`anonymousId` recipient. Any re-ingest DERIVED from a refused event must inherit the refusal, or it re-resolves the anonymous key as an EXTERNAL one and mints a row with `external_id = <anonId>` (worse than the ghost: that row trips `collidesWithIdentified` and locks the visitor out of their own feed). When the visitor later identifies, `adoptOrphanHistory` stamps the anon-keyed events, journey states, bucket memberships, sends and preferences with the contact's `contact_id` (`WHERE user_id = <key> AND contact_id IS NULL` — rows never move; reads follow the stamp), so nothing orphans.
...
### Account linking (Steam/Twitch → contact)

Binds a player's third-party platform account to a contact via `defineAccountLink()`. Driven by the game-publisher case: the platform knows a SteamID, the CRM knows an email, and nothing joined them. Full docs: `docs/account-links.md`; public guide at `apps/docs/content/docs/guides/account-links.mdx`.

- **Core** — the `AccountLinkProvider` contract + `oauth2Link()` / `steamOpenIdLink()` presets (`@hogsend/core`, `providers/account-link.ts`). Providers are CONFIG, not packages: a provider is a URL, one fetch and a field mapping, so `oauth2Link()` covers most platforms in ~15 lines. `RESERVED_ACCOUNT_LINK_IDS` blocks ids that would shadow a route (`me`, `start`, `callback`, `manage`, …).
- **DB** — `linked_accounts` (migration `0072`), three partial-unique indexes, `version bigint NOT NULL`. A soft-unlink sets `unlinked_at`; history is retained, so a pair with no LIVE row 404s rather than resurrecting.
- **Store** — `lib/account-links.ts`: `linkAccount`/`unlinkAccount` under a `pg_advisory_xact_lock`, monotonic `version` per `(provider, providerUserId)` (`COALESCE(MAX(version),0)+1` across live AND unlinked rows), `multiple`/`onConflict` policy, hook invocation. **The store NEVER emits** (DECISIONS §15.7) — it returns mutation facts and its callers emit. Two tests pin that: one scanning for the emit symbols, one pinning the store's runtime import list, so a future emit surface still has to import its way past them.
- **Providers** — `packages/engine/src/account-links/`: Steam (OpenID 2.0, bespoke `check_authentication` round-trip to Steam's HARDCODED endpoint, never `openid.op_endpoint` from the callback) and Twitch (OAuth2). **Steam needs NO credentials** and registers on any operator intent; `STEAM_WEB_API_KEY` is optional and only widens it with persona name + avatar. Twitch needs BOTH `ACCOUNT_LINK_TWITCH_CLIENT_ID` and `_SECRET` or it silently does not register. Discord is deliberately OUT: it already links via `plugin-discord`, and a second writer on `contacts.discordId` would drift.
- **Registration is on operator INTENT** (`lib/account-links-from-env.ts`) — any `ACCOUNT_LINK_*` var, `STEAM_WEB_API_KEY`, or any `accountLinks` option (even `{}`). No intent means no providers and no boot throw.
- **Hosted flow** — `routes/accounts/{start,callback}.ts`: `account_link` state purpose, PKCE, nonce burn, Redis throttle (fail-closed), `beforeLink` veto. Both routes are UNAUTHENTICATED by construction.
- **Data plane** — `/v1/accounts/*`: list, reverse lookup, `DELETE` unlink, insert-only `import`, `mint-link` (secret), plus `me`, `me/revoke` and `link-url` (userToken-gated). New ORTHOGONAL `accounts` scope. `GET /v1/accounts/me` NEVER confirms existence: absent/malformed/expired/forged all return `200 {"accounts":[]}`.
- **Guards BRANCH, never stack** (`routes/index.ts`), and the branch is gated on METHOD as well as path. Hono runs every matching `use` and `route("/v1", v1)` flattens middleware, so a blanket `/accounts/*` guard would 401 the hosted flow and 403 the player revoke — and a segment-only exemption let `DELETE` on a GET-only path reach the operator unlink with no key.
- **`version` is a bigint serialized as a decimal STRING at every boundary.** Nothing on any plane parses it. A value above `Number.MAX_SAFE_INTEGER` rounded through float64 breaks the customer's `incoming.version > stored.version` guard invisibly, which is the exact case that guard exists for.
- **Outbound** — `account.linked` / `account.unlinked` / `account.link_failed`, full current state, dedupe key `al:<provider>:<uid>:v<version>`, emitted from the INTENT layer only. A relink or a singleton displacement emits the unlink at the LOWER version FIRST, chained (not two racing `void` calls), so an out-of-order delivery is discarded by the monotonic guard rather than recording the wrong owner.
- **Journey plane** — `lib/account-link-ingest.ts` re-ingests all three events, so `defineJourney({ trigger: { event: "account.linked" } })` fires. It sits BESIDE the outbound emit and neither plane may be collapsed into the other: outbound ships state to the customer's subscriber, the journey plane writes `user_events` + pushes to Hatchet. Identity is the engine-internal `contactId` PIN, never the bare canonical key — `owner.userId` is `contactKey()`, so on a cold link it is an ANONYMOUS id and passing it alone mints `external_id = <anonId>` (the ghost-contact law). `link_failed` carries `allowCreate: false`, and its cold arm is `collidesWithIdentified`-screened because that key arrives on an unauthenticated URL.
- **NOT built yet**: the hosted manage page, embed SDK button, `@hogsend/client` `accounts.*`, property-sync cron, and Studio panel.

### Referrals (referrer → referee edges)

A referral is an edge between two contacts the engine already owns, authored
with `defineReferral()`. No second identity graph: the touch rides the link
tracker, the bind rides identity adoption, revenue rides `conversions`. Full
docs: `docs/referrals.md`; public guide at
`apps/docs/content/docs/guides/referrals.mdx`; design record `docs/prds/05-referrals.md`.

- **Core** — `defineReferral()` + `ReferralHooks` (`@hogsend/core`, `providers/referral.ts`): `{ id?, link: { destination, slugFrom?, campaign? }, qualify?: { event, where? }, bindWindow?, beforeTouch?, beforeBind?, beforeQualify? }`. Default id `"default"`.
- **Link type `shared`** — `links.owner_contact_id` + `links.referral_id`. Owned by a person, clicked by someone else: attributes to the owner, stitches NOBODY. Never reuse `personal` (it would identify every referee as the referrer). `mintLink` enforces the invariant; `getReferralLink({ referral?, contactId })` is THE one mint path, journey-safe (zero durable calls) and idempotent.
- **DB** — `referral_touches` (migration 0073) is the edge log: `status` `touched|bound|qualified|rejected`, `rejected_reason` `self|window|veto|bot|duplicate`, partial-unique `(referral_id, referee_contact_id, referrer_contact_id) WHERE status <> 'rejected'`. `attribution_credits.referral_touch_id` puts referral credit in the marketing ledger under `channel = "referral"`. Nothing weighted is persisted.
- **Store never emits** — `lib/referrals.ts` mutates, `lib/referral-intent.ts` is the ONE place that mutates AND emits (same law as `lib/account-links.ts`, pinned by tests). The "after" of every stage is a journey on the event.
- **Events** — `referral.touched|bound|qualified|converted|tree_converted|rejected`, in `WEBHOOK_EVENT_TYPES` and both vendored copies. `tree_converted` carries `{ level, viaContactId }` as FACTS, so a reward journey filters with `trigger.where: (b) => b.prop("level").eq(1)`; the bus copy never carries `value`, so the wildcard revenue conversion cannot fire for an ancestor. `referral.touched` carries `allowCreate: false` and every re-ingest carries the `contactId` PIN, never the bare anon key (the ghost-contact law).
- **The report is a query** (`lib/referral-report.ts`) — model / window / depth / weights are REQUEST parameters, never program config; nothing is backfilled. `REFERRAL_MODELS` = `first_touch|last_touch|linear|time_decay|position` (engine-owned, deliberately NOT `@hogsend/attribution`'s ids). Credit = `levelWeight[L] * Π(edgeWeight over L hops) * value`; a zero-weight edge is DROPPED, not kept at zero. Depth cap 5, one recursive CTE with an array cycle guard. **Currencies are never converted**: every monetary field is `{ currency, value }[]` (null currency ⇒ ISO 4217 `XXX`); the leaderboard's cross-currency sum is a sort key only. `getReferralTree` is a LEDGER view: no window, no weights.
- **Routes** — `POST /v1/referrals/touch`, `POST /v1/referrals/import` (insert-only, calls the store directly so it is SILENT), `GET /v1/referrals/report`, `GET /v1/referrals/tree/:contactId` under the new ORTHOGONAL `referrals` scope; `GET /v1/referrals/me` (publishable-or-secret + a server-minted `userToken`, NEVER confirms existence: absent/forged/expired all return `200 {"link":null,"stats":null}`). Guards BRANCH, never stack: `/referrals/me` is its own `use` before the secret list and there is no `/referrals/*` catch-all. Admin read-only surface at `GET /v1/admin/referrals` + `/v1/admin/referrals/overview` (definition read back from code + ledger funnel/sources/rejections/series, `getReferralOverview`) + `/v1/admin/referrals/:contactId` (the same report functions plus contact identity, the referrer's share links and the touch log). `overview` is a literal segment and MUST stay registered before `/{contactId}`.
- **SDKs** — `@hogsend/client` `referrals.touch/report/tree/import` (no `link()`: minting is browser-side), `@hogsend/js` `captureRef()` (`hs_ref` → `/v1/t/arrive`) + `referral.link()`, `@hogsend/react` `useReferralLink()`, MCP `get_referral_report` / `get_referral_tree`. Studio `Referrals` (definition strip, stat tiles, funnel, trend, sources, rejections, leaderboard; period/model/window/depth are request params) + detail (share links, indented tree, filtered touch log) are observe-only.
- **NOT built**: payouts (Stripe Connect, clawbacks, KYC, FX), a built-in per-referrer rate cap, discount codes, fingerprint fraud heuristics, group-level referrals, Studio authoring.

...
- **Routes** — `POST /v1/referrals/touch`, `POST /v1/referrals/import` (insert-only, calls the store directly so it is SILENT), `GET /v1/referrals/report`, `GET /v1/referrals/tree/:contactId` under the new ORTHOGONAL `referrals` scope; `GET /v1/referrals/me` (publishable-or-secret + a server-minted `userToken`, NEVER confirms existence: absent/forged/expired all return `200 {"link":null,"stats":null}`). Guards BRANCH, never stack: `/referrals/me` is its own `use` before the secret list and there is no `/referrals/*` catch-all. Admin read-only surface at `GET /v1/admin/referrals` + `/v1/admin/referrals/:contactId` (the same report functions plus contact identity and the touch log).
- **SDKs** — `@hogsend/client` `referrals.touch/report/tree/import` (no `link()`: minting is browser-side), `@hogsend/js` `captureRef()` (`hs_ref` → `/v1/t/arrive`) + `referral.link()`, `@hogsend/react` `useReferralLink()`, MCP `get_referral_report` / `get_referral_tree`. Studio `Referrals` + detail are observe-only.

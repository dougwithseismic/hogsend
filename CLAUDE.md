...
- **`JourneyContext`** (`src/journeys/journey-context.ts`) provides only durable execution primitives: `ctx.sleep()`, `ctx.checkpoint()`, `ctx.event.check()`, `ctx.event.fire()`. Service integrations (email via `sendJourneyEmail`, PostHog via `@hogsend/plugin-posthog`) are imported directly, not through the context.

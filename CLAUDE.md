...
SMS is a first-class channel mirroring the email architecture. Full docs: `docs/sms.md`. **Opt-in**: with no provider configured the SMS service is an inert stub and `sendSms` throws — existing deploys without Twilio creds are unaffected.

- **Contract** — `SmsProvider` in `@hogsend/core` (`packages/core/src/providers/sms.ts`, `defineSmsProvider()`). A dumb wire: plain-text `send(SendSmsOptions) → { id }`, `verifyWebhook({ payload, headers, url }) → SmsEvent` (the `url` is passed because Twilio signs the public URL + form params), `parseWebhook`. `SmsEvent` types: `sms.sent | sms.delivered | sms.failed | sms.inbound`. `meta` is REQUIRED (no pre-registry back-compat).
- **Plugin** — `packages/plugin-twilio` (`createTwilioProvider`), the reference. HTML never crosses the wire. NOTE: `twilio` v6 is CommonJS — use the default import (`import twilio from "twilio"`), not named `{ Twilio }`/`{ validateRequest }` (named imports type-check but throw at runtime).
...
- **Engine** — `container.ts` builds `SmsProviderRegistry` + resolves the active provider (`sms.defaultProvider ?? SMS_PROVIDER ?? "twilio"`, or the sole provider when only one is registered; unresolvable explicit id throws). `createTrackedSmsSender` (`lib/sms-mailer.ts` + `lib/sms-tracked.ts`) mirrors the email pipeline: idempotency short-circuit → dual-track suppression (`sms_suppressions` phone list + `email_preferences` `sms` channel/topic gate) → SMS frequency cap → STOP footer → test-mode redirect (`HOGSEND_TEST_PHONE`) → `sms_sends` write → `provider.send` → `sms.sent` outbound. `sendSms()` (`lib/sms.ts`) is the journey helper, replay-safe via the `smsSend` key kind (disjoint from email's `send`). `ctx.history.sms({ phone, template })` mirrors `ctx.history.email`.
- **Webhook** — `POST /v1/webhooks/sms/:providerId` (`routes/webhooks/sms-provider.ts`), registered before the `:sourceId` catch-all; `"sms"` is a reserved connector/source id. Inbound STOP/START/HELP handled by `lib/sms-inbound.ts` (dual-track suppression, `contact.unsubscribed` emit on opt-out; confirmation replies default OFF).
- **DB** — `sms_sends`, `sms_suppressions`, `contacts.phone` (partial-unique live index, like `discordId`). Phone is NOT yet a merge-participating identity `Kind` — STOP resolves a contact by a direct `contacts.phone` lookup.
